...
function defineStyles() {
    ...
    '#clinical-timelines .hidden {' + '    display: none !important;' + '}', '#clinical-timelines .highlighted {' + '    border: 2px solid black !important;' + '    cursor: pointer !important;' + '    border-radius: 4px !important;' + '    padding: 5px !important;' + '    stroke: black;' + '    stroke-width: 3px;' + '}',
    ...
    //Annotations
    '#clinical-timelines > #left-side > .annotation {' + '    font-size: 16px;' + '    text-align: right;' + '}', '#clinical-timelines > #left-side > .annotation .stats,' + '#clinical-timelines > #left-side > .annotation #participant,' + '#clinical-timelines > #left-side > .annotation .characteristic span {' + '    font-weight: bold;' + '}',

    //Controls
    '#clinical-timelines > #left-side > .wc-controls {' + '    margin-bottom: 0;' + '    clear: left;' + '}', '#clinical-timelines > #left-side > .wc-controls .control-group {' + '    margin-bottom: 5px;' + '    display: block;' + '    float: right;' + '    clear: both;' + '}', '#clinical-timelines > #left-side > .wc-controls .control-group > * {' + '    display: inline-block;' + '    vertical-align: top;' + '}', '#clinical-timelines > #left-side > .wc-controls .control-group .changer {' + '    margin-left: 5px;' + '}', '#clinical-timelines > #left-side > .wc-controls .back-button button {' + '    padding: 0 5px;' + '    font-size: 14px;' + '    float: left;' + '    clear: left;' + '    margin-top: 5px;' + '}',
...
function onInit() {
    ...
    this.controls.config.inputs = this.controls.config.inputs.filter(function (input) {
        if (input.type !== 'subsetter') {
            ...
            if (input.label === 'Event Type' && input.description === 'highlighting') input.values = _this.config.color_dom;else if (input.label === 'Y-axis' && input.description === 'grouping') input.values = _this.config.groupings.map(function (grouping) {
    ...
    if (this.raw_data[0].hasOwnProperty(this.config.tooltip_col)) this.config.marks.forEach(function (mark) {
...
function drawParticipantTimeline() {
    ...
    webcharts.multiply(this.participantTimeline, longParticipantData.filter(function (d) {
    ...
    this.listing.draw(wideParticipantData.filter(function (d) {
        return _this.currentEventTypes !== 'All' ? _this.currentEventTypes.indexOf(d[_this.config.event_col]) > -1 : true;
    }));
}
...
function augmentOtherControls() {
    ...
    otherControls.each(function (d) {
        ...
        if (d.label === 'Y-axis') {
            ...
            if (d.description === 'sort') options.property('label', function (di) {
...
function onLayout() {
    ...
    this.controls.wrap.selectAll('.control-group').each(function (d) {
        var controlGroup = d3.select(this),
            ...
            container = controlGroup.insert('div', ':first-child').classed('label-description', true);
    ...
    this.participantDetails.wrap = this.leftSide.insert('div', ':first-child').classed('annotation participant-details hidden', true);
    this.participantDetails.wrap.append('div').html(this.config.id_unitPropCased + ': <span id = \'participant\'></span>');
...
function legendFilter() {
    ...
    eventTypes = eventTypeControl.selectAll('.changer option').sort(function (a, b) {
        ...
        // event type options
    legendItems = this.wrap.selectAll('.legend-item').classed('hidden', function (d) {
        return d.label === 'None';
    ...
    legendItems.on('click', function (d) {
        ...
        if (context.selected_id) drawParticipantTimeline.call(context);else context.draw();
    });
}
...
function drawOngoingMarks() {
    ...
    this.svg.selectAll('.line-supergroup .line').filter(function (d) {
        ...
        g.append('polygon').classed('ongoing-event', true).attr({
...
function participantTimeline(clinicalTimelines) {
    var participantTimeline = webcharts.createChart(clinicalTimelines.rightSide.node(), clinicalTimelines.config.participantSettings);
