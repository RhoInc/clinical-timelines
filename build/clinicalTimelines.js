...
function setDefaultTimeRanges() {
    ...
    this.date_range = this.config.date_range instanceof Array && this.config.date_range.length === 2 && this.config.date_range[0].toString() !== this.config.date_range[1].toString() && this.config.date_range.every(function (date) {
        return date instanceof Date || d3$1.time.format(_this.config.date_format).parse(date);
    }) ? this.config.date_range.map(function (date) {
        return date instanceof Date ? date : d3$1.time.format(_this.config.date_format).parse(date);
...
function controlGroupLayout() {
    ...
    this.controls.wrap.selectAll('.control-group').each(function (d) {
        ...
        if (d.value_col === context.config.id_col) context.controls.wrap.insert('div', ':first-child').classed('ct-controls ct-horizontal-rule', true).text('Controls');else if (d.option === 'y.grouping') {
...
function timeRangeControl(datum) {
    ...
    timeRangeInput.on('change', function (d) {
        ...
        var input = context.config.time_scale === 'date' ? d3$1.time.format('%Y-%m-%d').parse(this.value) : +this.value;

        if (d.index === 0 && input >= context[time_range][1]) input = context.config.time_scale === 'date' ? new Date(context[time_range][1].getTime() - increment) : context[time_range][1] - increment;else if (d.index === 1 && input <= context[time_range][0]) input = context.config.time_scale === 'date' ? new Date(context[time_range][0].getTime() + increment) : input = context[time_range][0] + increment;
...
function updateTimeRangeControls() {
    ...
    timeRangeControls.property('value', function (d) {
        return _this.config.time_scale === 'date' ? d3$1.time.format('%Y-%m-%d')(_this.time_range[d.index]) : +_this.time_range[d.index];
    });
}
