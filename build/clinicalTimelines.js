...
function syncRendererSpecificSettings(settings) {
    ...
    //date_ranges
    if (settings.date_range && settings.date_ranges === null) settings.date_ranges = [settings.date_range];
    if (settings.date_ranges) settings.date_ranges = settings.date_ranges.filter(function (date_range) {
        return date_range instanceof Array && date_range.length === 2 && date_range[0].toString() !== date_range[1].toString() && date_range.every(function (date) {
            return date instanceof Date || d3.time.format(settings.date_format).parse(date);
        });
    }).map(function (date_range) {
        return {
            label: date_range.map(function (date) {
            ...
            time_range: date_range.map(function (date) {
                return date instanceof Date ? d3.time.format(settings.date_format)(date) : date;
            }).join(' - ')
        };
    });else settings.date_ranges = [];

    //day_ranges
    if (settings.day_range && settings.day_ranges === null) settings.day_ranges = [settings.day_range];
    if (settings.day_ranges) settings.day_ranges = settings.day_ranges.filter(function (day_range) {
        return day_range instanceof Array && day_range.length === 2 && day_range[0].toString() !== day_range[1].toString() && day_range.every(function (day) {
...
function timeRange(dropdown, d) {
    var option = d3.select(dropdown).selectAll('option').filter(function () {
...
function augmentOtherControls() {
    ...
    controls.filter(function (d) {
        ...
        d3.select(this).selectAll('option').property('label', function (di) {
            var time_range = time_ranges.splice(time_ranges.findIndex(function (dii) {
...
function onChange(input, d) {
    ...
    customTimeRange.time_range = customTimeRange.domain.map(function (d) {
        if (_this.config.time_scale === 'date') return d3.time.format(_this.config.date_format)(d);else return '' + d;
    }).join(' - ');
    ...
    this.controls.wrap.selectAll('.control-group').filter(function (d) {
        ...
        return this.label === 'user input';
    }).datum(customTimeRange.time_range).text(function (d) {
        return d;
    }).node();
...
function updatePopulationDetails() {
    this.clinicalTimelines.containers.popCurrentFilters.html('<span class = \'ct-stats ct-sample\'>' + this.populationDetails.n + '</span> of <span class = \'ct-stats\'>' + this.populationDetails.N + '</span> ' + (this.populationDetails.N > 1 ? this.config.id_unitPlural : this.config.id_unit) + ' (<span class = \'ct-stats\'>' + d3.format('%')(this.populationDetails.rate) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' have data that meet the current filter criteria.\'>&#9432;</span>');
    ...
    IDdata.forEach(function (IDdatum) {
        ...
        if (overlappingLines.length) {
            ...
            overlappingLines.forEach(function (currentLine, i) {
                ...
                var g = d3.select(_this.clinicalTimelines.document.getElementsByClassName(className)[0]);
                g.attr('transform', currentLine.offset > 0 ? 'translate(0,' + currentLine.offset * +mark.attributes['stroke-width'] * 1.5 + ')' : 'translate(0,0)');
            });
        }
    });
}
...
function offsetOverlappingMarks() {
    ...
    this.config.marks.forEach(function (mark, i) {
        ...
        if (mark.type === 'line') {
            ...
            if (_this.config.ongo_col) markData.forEach(function (d) {
