...
function defineStyles() {
    ...
    //Reference Tables
    '#clinical-timelines > #left-side .ct-reference-line-header {' + '    text-align: center;' + '    border-bottom: 1px solid black;' + '    padding-bottom: 5px;' + '}', '#clinical-timelines > #left-side .ct-reference-line-table {' + '    width: 100%;' + '    display: table;' + '}', '#clinical-timelines > #left-side .ct-reference-line-table th,' + '#clinical-timelines > #left-side .ct-reference-line-table td {' + '    text-align: left;' + '}', '#clinical-timelines > #left-side .ct-higher-level {' + '    border-bottom: 1px dotted lightgray;' + '    font-weight: bold;' + '    font-size: 14px;' + '}', '#clinical-timelines > #left-side .ct-lower-level {' + '    font-size: 12px;' + '}', '#clinical-timelines > #left-side .ct-lower-level.ct-indent {' + '    padding-left: 5%;' + '}',
    ...
    //Lines
    '#clinical-timelines path.wc-data-mark {' + '    stroke-width: 4;' + '    stroke-opacity: 1;' + '}', '#clinical-timelines path.wc-data-mark.highlighted {' + '    stroke-width: 7;' + '}', '#clinical-timelines line.highlight-overlay {' + '    stroke-width: 2;' + '    stroke-linecap: round;' + '}',

    //Circles
    '#clinical-timelines circle.wc-data-mark {' + '    stroke-width: 0;' + '    fill-opacity: 1;' + '}', '#clinical-timelines circle.wc-data-mark.highlighted {' + '    stroke-opacity: 1;' + '    stroke-width: 2;' + '}',

    //Arrows
    '#clinical-timelines polygon.ongoing-event {' + '}', '#clinical-timelines polygon.ongoing-event.highlighted {' + '    stroke-width: 2;' + '}',

    //Reference lines
    '#clinical-timelines .wc-chart .wc-svg title {' + '    white-space: pre;' + '}', '#clinical-timelines > #right-side .wc-chart .wc-svg .visible-reference-line {' + '    stroke: black;' + '    stroke-width: 2;' + '    stroke-dasharray: 2,2;' + '}', '#clinical-timelines > #right-side .wc-chart .wc-svg .visible-reference-line.hover {' + '    stroke-dasharray: none;' + '}', '#clinical-timelines > #right-side .wc-chart .wc-svg .invisible-reference-line {' + '    stroke: black;' + '    stroke-width: 20;' + '    stroke-opacity: 0;' + '}', '#clinical-timelines > #right-side .wc-chart .wc-svg .reference-line-label-box {' + '    fill: white;' + '    stroke: black;' + '    stroke-width: black;' + '}',
...
function updateTimeRangeControls() {
    ...
    if (!document.documentMode) timeRangeControls.property('type', this.config.time_scale === 'date' ? 'date' : 'number');
...
function updatePopulationDetails() {
    var sample = '<span class = \'ct-stats sample\'>' + this.populationDetails.n + '</span> of <span class = \'ct-stats\'>' + this.populationDetails.N + '</span> ' + (this.populationDetails.N > 1 ? this.config.id_unitPlural : this.config.id_unit) + ' (<span class = \'ct-stats\'>' + d3.format('%')(this.populationDetails.rate) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' have data that meet the current filter criteria.\'>&#9432;</span>',
        sampleInsideTimeRange = this.populationDetails.nInsideTimeRange < this.populationDetails.n ? '<span class = \'ct-stats sample-inside-time-range\'>' + this.populationDetails.nInsideTimeRange + '</span> of <span class = \'ct-stats sample\'>' + this.populationDetails.n + '</span> displayed (<span class = \'ct-stats\'>' + d3.format('%')(this.populationDetails.rateInsideTimeRange) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' have events that occur in the current time range.\'>&#9432;</span>' : '',
        sampleOutsideTimeRange = this.populationDetails.nOutsideTimeRange ? '<span class = \'ct-stats sample-outside-time-range\'>' + this.populationDetails.nOutsideTimeRange + '</span> of <span class = \'ct-stats sample\'>' + this.populationDetails.n + '</span> hidden (<span class = \'ct-stats\'>' + d3.format('%')(this.populationDetails.rateOutsideTimeRange) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' do not have events that occur in the current time range.\'>&#9432;</span>' : '';
...
function addStriping() {
    ...
    var yAxisGridLines = this.svg.selectAll('.y.axis .tick').each(function (d, i) {
        d3.select(this).select('text').attr('dy', context.y.rangeBand() / 2);
        d3.select(this).insert('rect', ':first-child').attr({
            id: d,
...
function drawReferenceTable(reference_line) {
    ...
    reference_line.container = this.leftSide.append('div').classed('ct-reference-line-container', true);
    reference_line.container.append('h3').classed('ct-reference-line-header', true).text(reference_line.label);

    //Add reference line table table.
    reference_line.table = reference_line.container.append('table').classed('ct-reference-line-table', true);
...
function drawReferenceLines() {
    ...
    if (this.config.reference_lines) {
        ...
        var referenceLinesGroup = this.svg.insert('g', '#clinical-timelines .wc-chart .wc-svg .line-supergroup').classed('reference-lines', true);
        ...
        this.config.reference_lines.filter(function (reference_line) {
            ...
            var referenceLineGroup = referenceLinesGroup.append('g').classed('reference-line', true).attr('id', 'reference-line-' + i),
                timepoint = _this.config.time_function(reference_line.timepoint),
                x = _this.x(timepoint),
                y2 = _this.plot_height + (_this.config.y.column === _this.config.id_col ? _this.y.rangeBand() : 0),
                visibleReferenceLine = referenceLineGroup.append('line').classed('visible-reference-line', true).attr({
                x1: x,
                x2: x,
                y1: 0,
                y2: y2,
                ...
                invisibleReferenceLine = referenceLineGroup.append('line').classed('invisible-reference-line', true).attr({
                x1: x,
                x2: x,
                y1: 0,
                y2: y2,
            ...
            direction = reference_line.timepoint <= (_this.x_dom[1] - _this.x_dom[0]) / 2 ? 'right' : 'left',
                referenceLineLabel = referenceLineGroup.append('text').classed('reference-line-label', true).attr({
                x: x,
                y: 0,
                ...
                referenceLineLabelBox = referenceLineGroup.insert('rect', '.reference-line-label').classed('reference-line-label-box', true).attr({
                x: dimensions.x - 10,
                y: dimensions.y - 5,
...
function drawTimeRange() {
    ...
    var x_dom = this.x_dom.map(function (x) {
        ...
        return d3.time.format(_this.parent.clinicalTimelines.config.date_format)(dt);
