...
function syncRendererSpecificSettings(settings) {
    ...
    //event_symbols
    if (settings.event_symbols instanceof Array) settings.event_symbols = settings.event_symbols.filter(function (event_symbol) {
        return ['square', 'diamond', 'triangle'].indexOf(event_symbol.symbol) > -1;
    });
    if (!(settings.event_symbols instanceof Array && settings.event_symbols.length)) delete settings.event_symbols;
    ...
    if (Array.isArray(settings.date_range) && settings.date_ranges === null) settings.date_ranges = [{
        domain: settings.date_range,
        label: 'Initial Date Range'
    }];else if (Array.isArray(settings.date_range) && Array.isArray(settings.date_ranges)) settings.date_ranges.unshift(settings.date_range);

    if (Array.isArray(settings.date_ranges)) settings.date_ranges = settings.date_ranges.filter(function (date_range) {
        ...
        return domain instanceof Array && domain.length === 2 && domain[0].toString() !== domain[1].toString() && domain.every(function (d) {
            return d instanceof Date || d3$1.time.format(settings.date_format).parse(d);
        });
    }).map(function (date_range) {
        var domain = (date_range.domain || date_range).map(function (date) {
            return date instanceof Date ? date : d3$1.time.format(settings.date_format).parse(date);
        });
        var label = date_range.label || domain.map(function (date) {
            return date instanceof Date ? d3$1.time.format(settings.date_display_format)(date) : date;
        }).join(' - ');
        return {
            domain: domain,
            label: label
    ...
    if (Array.isArray(settings.day_range) && settings.day_ranges === null) settings.day_ranges = [{
        domain: settings.day_range,
        label: 'Initial Day Range'
    }];else if (Array.isArray(settings.day_range) && Array.isArray(settings.day_ranges)) settings.day_ranges.unshift(settings.day_range);

    if (settings.day_range && settings.day_ranges === null) settings.day_ranges = [{
        domain: settings.day_range,
        label: 'Initial Day Range'
    }];
    if (Array.isArray(settings.day_ranges)) settings.day_ranges = settings.day_ranges.filter(function (day_range) {
    ...
    if (settings.reference_lines) {
        ...
        settings.reference_lines = settings.reference_lines.map(function (reference_line) {
            ...
            return reference_line.time_scale === 'Day' && Number.isInteger(reference_line.timepoint) || reference_line.time_scale === 'Date' && d3$1.time.format(settings.date_format).parse(reference_line.timepoint) instanceof Date;
        });
...
function syncTimeScaleSettings(settings) {
    settings.time_scale = ['date', 'day'].indexOf(settings.time_scale.toLowerCase()) > -1 ? settings.time_scale.substring(0, 1).toUpperCase() + settings.time_scale.substring(1).toLowerCase() : 'Date';
...
function syncControls(controls, settings) {
    ...
    var syncedControls = d3$1.merge([[settings.filters[0]], // ID dropdown first
...
function defineStyles() {
    ...
    //buttons
    '#clinical-timelines .ct-button {' + '    display: inline-block;' + '    padding: 3px 5px !important;' + '    border: 2px solid black !important;' + '    border-radius: 4px !important;' + '    color: #333;' + '    background: #ccc;' + '    cursor: pointer !important;' + '}', '#clinical-timelines .ct-button:hover {' + '    color: #ccc;' + '    background: #333 !important;' + '}',

    /***--------------------------------------------------------------------------------------\
      Left and right columns
    \--------------------------------------------------------------------------------------***/

    '#clinical-timelines .ct-column {' + '    display: inline-block;' + '}', '#clinical-timelines #ct-left-column {' + '    width: 20%;' + '    float: left;' + '}', '#clinical-timelines #ct-right-column {' + '    width: 79%;' + '    float: right;' + '}', '#clinical-timelines .ct-column > * {' + '    width: 100%;' + '    vertical-align: top;' + '    display: inline-block;' + '    margin-bottom: 10px;' + '    border: 1px solid #eee;' + '}', '#clinical-timelines .ct-column > * > * {' + '    margin: 10px;' + '}',
    ...
    //Arrows
    '#clinical-timelines polygon.ct-ongoing-event {' + '}', '#clinical-timelines polygon.ct-ongoing-event.ct-highlighted {' + ('    stroke-width: ' + line.attributes['stroke-width'] / 3 + ';') + '}',
...
function exportData() {
    ...
    var CSV = new Blob([CSVarray.join('\n')], { type: 'text/csv;charset=utf-8;' }),
        fileName = 'ClinicalTimelinesData_' + d3$1.time.format('%Y-%m-%dT%H-%M-%S')(new Date()) + '.csv',
...
function defineLayout() {
    ...
    this.containers.main = d3$1.select(this.element).append('div').attr('id', 'clinical-timelines');
    ...
    this.containers.exportButton = this.containers.populationDetails.append('div').classed('ct-button', true).attr('id', 'ct-export-button').append('a').html('&dArr; Data').on('click', function () {
    ...
    this.containers.backButton = this.containers.IDdetails.append('div').classed('ct-button', true).attr('id', 'ct-back-button').append('a').html('&lArr; Back').on('click', function () {
...
function standardizeTimeVariables() {
    ...
    this.initial_data.forEach(function (d, i) {
        ...
        if (has_stdt) {
            if (!d3$1.time.format(_this.config.date_format).parse(d[_this.config.stdt_col])) d[_this.config.stdt_col] = '';
        }
        if (has_endt) {
            if (!d3$1.time.format(_this.config.date_format).parse(d[_this.config.endt_col])) d[_this.config.endt_col] = d[_this.config.stdt_col];
        }
        if (has_stdy) {
...
function handleEventTypes() {
    ...
    this.allEventTypes = d3$1.set(this.initial_data.map(function (d) {
...
function defineFullTimeRanges() {
    ...
    if (this.anyDates) {
        //full domain
        this.full_date_range = [d3$1.min(this.initial_data, function (d) {
    ...
    if (this.anyDays) {
        //full domain
        this.full_day_range = [d3$1.min(this.initial_data, function (d) {
...
function checkFilters() {
    ...
    this.controls.config.inputs = this.controls.config.inputs.filter(function (input) {
        if (input.type !== 'subsetter') return true;else if (!_this.initial_data[0].hasOwnProperty(input.value_col)) {
            ...
            var levels = d3$1.set(_this.initial_data.map(function (d) {
...
function IDdetails() {
    ...
    this.clinicalTimelines.containers.IDdetails.selectAll('div.characteristic').data(this.config.id_characteristics).enter().append('div').classed('ct-characteristic', true).html(function (d) {
...
function controlGroupLayout() {
    ...
    this.controls.wrap.selectAll('.control-group').each(function (d) {
        ...
        if (d.value_col === context.config.id_col) context.controls.wrap.insert('div', ':first-child').classed('ct-controls ct-horizontal-rule', true).text('Controls');else if (context.config.groupings.length && d.option === 'y.groupingLabel' || !context.config.groupings.length && d.option === 'y.sort') {
...
function drawIDtimeline() {
    ...
    if (this.config.id_characteristics) {
        ...
        this.clinicalTimelines.containers.IDdetails.selectAll('.ct-characteristic').each(function (d) {
            d3$1.select(this).select('span').text(id_characteristics[d.value_col]);
        });
    }
    ...
    }), this.config.event_col, this.config.color_dom, this.clinicalTimelines.test);
...
function eventHighlighting(select$$1, d) {
    ...
    this.config.event_highlighted = d3$1.select(select$$1).select('option:checked').text();
...
function timeScale(dropdown, d) {
    ...
    this.config.time_scale = d3$1.select(dropdown).select('option:checked').text();
...
function timeRange(dropdown, d) {
    var label = d3$1.select(dropdown).selectAll('option').filter(function () {
        return this.selected;
    }).text();
    var time_range = this.config[this.config.time_scale.toLowerCase() + '_ranges'].find(function (di) {
...
function onChange(input, d) {
    ...
    //User input.
    var inputValue = this.config.time_scale === 'Date' ? d3$1.time.format('%Y-%m-%d').parse(input.value) : +input.value;
...
function IDchange(select$$1) {
    ...
    this.selected_id = d3$1.select(select$$1).select('option:checked').text();
    this.filters.filter(function (filter) {
        return filter.col === _this.config.id_col;
...
function eventTypeChange(select$$1) {
    ...
    this.currentEventTypes = d3$1.select(select$$1).selectAll('select option:checked').pop().map(function (d) {
...
function updateTimeRangeControls$1() {
    ...
    timeRangeControls.property('value', function (d) {
        return _this.config.time_scale === 'Date' ? d3$1.time.format(_this.config.date_format)(_this.time_range[d.index]) : +_this.time_range[d.index];
    });
}
...
function updatePopulationDetails() {
    this.clinicalTimelines.containers.popCurrentFilters.html('<span class = \'ct-stats ct-sample\'>' + this.populationDetails.n + '</span> of <span class = \'ct-stats\'>' + this.populationDetails.N + '</span> ' + (this.populationDetails.N > 1 ? this.config.id_unitPlural : this.config.id_unit) + ' (<span class = \'ct-stats\'>' + d3$1.format('%')(this.populationDetails.rate) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' have data that meet the current filter criteria.\'>&#9432;</span>');
    this.clinicalTimelines.containers.popInsideTimeRange.html(this.populationDetails.nInsideTimeRange < this.populationDetails.n ? '<span class = \'ct-stats ct-sample-inside-time-range\'>' + this.populationDetails.nInsideTimeRange + '</span> of <span class = \'ct-stats ct-sample\'>' + this.populationDetails.n + '</span> displayed (<span class = \'ct-stats\'>' + d3$1.format('%')(this.populationDetails.rateInsideTimeRange) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' have events that occur in the current time range.\'>&#9432;</span>' : '');
    this.clinicalTimelines.containers.popOutsideTimeRange.html(this.populationDetails.nOutsideTimeRange ? '<span class = \'ct-stats ct-sample-outside-time-range\'>' + this.populationDetails.nOutsideTimeRange + '</span> of <span class = \'ct-stats ct-sample\'>' + this.populationDetails.n + '</span> hidden (<span class = \'ct-stats\'>' + d3$1.format('%')(this.populationDetails.rateOutsideTimeRange) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' do not have events that occur in the current time range.\'>&#9432;</span>' : '');
}
...
function definePopulationDetails() {
    ...
    this.populationDetails.sample = d3$1.set(this.filtered_wide_data.map(function (d) {
        return d[_this.config.id_col];
...
function defineGroupingData() {
    ...
    if (this.config.y.grouping) {
        ...
        this.groupings = d3$1.set(this.longDataInsideTimeRange.map(function (d) {
            return d[_this.config.y.grouping];
        })).values().sort().map(function (d, i) {
            var groupingObject = {
                key: d,
                IDs: []
            };
...
function sortYdomain() {
    ...
    if (this.config.y.sort === 'By Earliest Event') {
        if (this.config.y.grouping) {
            ...
            this.config.y.domain = d3$1.nest().key(function (d) {
                return d[_this.config.id_col];
            }).rollup(function (d) {
                return d3$1.min(d, function (di) {
        ...
        if (this.config.y.grouping) {
            ...
            this.config.y.domain = d3$1.set(this.longDataInsideTimeRange.map(function (d) {
                return d[_this.config.id_col];
...
function drawTopXaxis() {
    var topXaxis = d3$1.svg.axis().scale(this.x).orient('top').ticks(this.xAxis.ticks()[0]).tickFormat(this.config.x_displayFormat).innerTickSize(this.xAxis.innerTickSize()).outerTickSize(this.xAxis.outerTickSize()),
...
function addTooltip() {
    ...
    this.svg.on('mousemove', function () {
        context.tooltip.text('' + context.config.x_displayFormat(context.x.invert(d3$1.mouse(this)[0])));
    });
}
...
function addStriping() {
    ...
    var yAxisGridLines = this.svg.selectAll('.y.axis .tick').each(function (d, i) {
        //Offset tick label.
        d3$1.select(this).select('text').attr('dx', 4).attr('dy', context.y.rangeBand() / 4);

        //Insert a rectangle with which to visually group each ID's events.
        d3$1.select(this).insert('rect', ':first-child').classed('ct-stripe', true).attr({
            id: d,
...
function offsetCircles(mark, markData) {
    ...
    if (this.raw_data.length && this.raw_data[0].hasOwnProperty(this.config.offset_col)) {
        this.svg.selectAll('g.point').each(function (d) {
            d3$1.select(this).attr('transform', 'translate(0,' + d.offset * context.config.mark_thickness * 2 + ')');
        });
    } else {
        ...
        overlapping.forEach(function (d) {
            ...
            d.values.keys.forEach(function (di, i) {
                ...
                var g = d3$1.select(_this.clinicalTimelines.document.getElementsByClassName(className)[0]);
...
function offsetLines(mark, markData) {
    ...
    if (this.config.offset_col && this.raw_data.length && this.raw_data[0].hasOwnProperty(this.config.offset_col)) {
        ...
        var IDdata = d3$1.nest().key(function (d) {
...
function updateTable(reference_line) {
    ...
    reference_line.nested_data = d3$1.nest().key(function (d) {
...
function addDrag(reference_line) {
    var context = this,
        drag = d3$1.behavior.drag().origin(function (d) {
        return d;
...
function setClipPath() {
    ...
    this.marks.forEach(function (mark) {
        mark.groups.selectAll('circle,line,path,polygon,rect,text').attr('clip-path', 'url(#' + _this.id + ')');
    });
}
