...
function syncTimeScaleSettings(settings) {
    ...
    settings.x.label = settings.time_scale.substring(0, 1).toUpperCase() + settings.time_scale.substring(1);
...
function defineSettings() {
    ...
    this.settings.controls = defaults$1.syncControls(defaults$1.controls, clone(this.settings.synced));
}
...
function defineStyles() {
    ...
    //Lines
    '#clinical-timelines path.wc-data-mark {' + '    stroke-width: 4;' + '    stroke-opacity: 1;' + '}', '#clinical-timelines path.wc-data-mark.ct-highlighted {' + '    stroke-width: 7;' + '}', '#clinical-timelines line.ct-highlight-overlay {' + '    clip-path: url(#1);' + '    stroke-width: 3;' + '    stroke-linecap: round;' + '}',

    //Circles
    '#clinical-timelines circle.wc-data-mark {' + '    stroke-width: 0;' + '    fill-opacity: 1;' + '}', '#clinical-timelines circle.wc-data-mark.ct-highlighted {' + '    stroke-opacity: 1;' + '    stroke-width: 2;' + '}',

    //Arrows
    '#clinical-timelines polygon.ct-ongoing-event {' + '    clip-path: url(#1);' + '}', '#clinical-timelines polygon.ct-ongoing-event.ct-highlighted {' + '    stroke-width: 2;' + '}',

    //Reference lines
    '#clinical-timelines #ct-right-column .wc-chart .wc-svg .ct-visible-reference-line {' + '    stroke: black;' + '    stroke-width: 2;' + '    stroke-dasharray: 2,2;' + '}', '#clinical-timelines #ct-right-column .wc-chart .wc-svg .ct-visible-reference-line.ct-hover {' + '    stroke-dasharray: none;' + '}', '#clinical-timelines #ct-right-column .wc-chart .wc-svg .ct-invisible-reference-line {' + '    stroke: black;' + '    stroke-width: 20;' + '    stroke-opacity: 0;' + '}', '#clinical-timelines #ct-right-column .wc-chart .wc-svg .ct-reference-line-label-box {' + '    fill: white;' + '    stroke: black;' + '    stroke-width: black;' + '}',
...
function updatePopulationDetails() {
    var sample = '<span class = \'ct-stats ct-sample\'>' + this.populationDetails.n + '</span> of <span class = \'ct-stats\'>' + this.populationDetails.N + '</span> ' + (this.populationDetails.N > 1 ? this.config.id_unitPlural : this.config.id_unit) + ' (<span class = \'ct-stats\'>' + d3.format('%')(this.populationDetails.rate) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' have data that meet the current filter criteria.\'>&#9432;</span>',
        ...
        sampleOutsideTimeRange = this.populationDetails.nOutsideTimeRange ? '<span class = \'ct-stats ct-sample-outside-time-range\'>' + this.populationDetails.nOutsideTimeRange + '</span> of <span class = \'ct-stats ct-sample\'>' + this.populationDetails.n + '</span> hidden (<span class = \'ct-stats\'>' + d3.format('%')(this.populationDetails.rateOutsideTimeRange) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' do not have events that occur in the current time range.\'>&#9432;</span>' : '';
...
function drawReferenceTable(reference_line) {
    ...
    reference_line.container = this.clinicalTimelines.containers.leftColumn.append('div').classed('ct-reference-line-table-container', true);
    reference_line.container.append('h3').classed('ct-reference-line-table-header', true).text(reference_line.label);

    //Add reference line table table.
    reference_line.table = reference_line.container.append('div').classed('ct-reference-line-table-body', true).append('table');
    reference_line.table.append('tbody').selectAll('tr').data(reference_line.flattened_data).enter().append('tr').each(function (d) {
...
function drawReferenceLines() {
    ...
    if (this.config.reference_lines) {
        ...
        var referenceLinesGroup = this.svg.insert('g', '#clinical-timelines .wc-chart .wc-svg .line-supergroup').classed('ct-reference-lines', true);
        ...
        this.config.reference_lines.filter(function (reference_line) {
            ...
            var referenceLineGroup = referenceLinesGroup.append('g').classed('ct-reference-line', true).attr('id', 'ct-reference-line-' + i);
            var timepoint = _this.config.time_function(reference_line.timepoint);
            var x = _this.x(timepoint);
            var y2 = _this.plot_height + (_this.config.y.column === _this.config.id_col ? _this.y.rangeBand() : 0);
            var visibleReferenceLine = referenceLineGroup.append('line').classed('ct-visible-reference-line', true).attr({
                x1: x,
                x2: x,
                y1: 0,
                y2: y2,
            ...
            var invisibleReferenceLine = referenceLineGroup.append('line').classed('ct-invisible-reference-line', true).attr({
                x1: x,
                x2: x,
                y1: 0,
                y2: y2,
            ...
            var referenceLineLabel = referenceLineGroup.append('text').classed('ct-reference-line-label', true).attr({
                x: x,
            ...
            var referenceLineLabelBox = referenceLineGroup.insert('rect', '.ct-reference-line-label').classed('ct-reference-line-label-box', true).attr({
                x: dimensions.x - 10,
                y: dimensions.y - 5,
            ...
            invisibleReferenceLine.on('mouseover', function () {
