...
function defineStyles() {
    ...
    //Mark highlighting
    '#clinical-timelines .highlighted {' + '    stroke: black;' + '}', '#clinical-timelines path.highlighted {' + '    stroke-width: 5px;' + '}', '#clinical-timelines line.highlight-overlay {' + '    stroke-width: 2px;' + '}', '#clinical-timelines circle.highlighted {' + '    stroke-width: 2px;' + '}', '#clinical-timelines polygon.highlighted {' + '    stroke-width: 2px;' + '}',
...
function syncSettings(settings) {
    ...
    syncedSettings.id_characteristics = arrayOfVariablesCheck(defaultId_characteristics, syncedSettings.id_characteristics);
    syncedSettings.id_unitPropCased = syncedSettings.id_unit.substring(0, 1).toUpperCase() + syncedSettings.id_unit.substring(1).toLowerCase();

    //Events
    if (!(syncedSettings.event_types instanceof Array && syncedSettings.event_types.length)) delete syncedSettings.event_types;

    //Filters
    var defaultFilters = [{ value_col: syncedSettings.id_col, label: syncedSettings.id_unitPropCased }, { value_col: syncedSettings.site_col, label: 'Site' }, { value_col: syncedSettings.event_col, label: 'Event Type' }];
    if (syncedSettings.ongo_col) defaultFilters.splice(2, 0, { value_col: syncedSettings.ongo_col, label: 'Ongoing?' });
    ...
    if (['horizontal', 'vertical'].indexOf(syncedSettings.grouping_direction) === -1) syncedSettings.grouping_direction = 'horizontal';

    //Reference lines
    if (syncedSettings.reference_lines) {
        if (!(syncedSettings.reference_lines instanceof Array)) syncedSettings.reference_lines = [syncedSettings.reference_lines];

        syncedSettings.reference_lines = syncedSettings.reference_lines.map(function (referenceLine) {
    ...
    //Details
    var defaultDetails = syncedSettings.time_scale === 'Study Day' ? [{ value_col: syncedSettings.event_col, label: 'Event Type' }, { value_col: syncedSettings.stdy_col, label: 'Start Day' }, { value_col: syncedSettings.endy_col, label: 'Stop Day' }, { value_col: syncedSettings.seq_col, label: 'Sequence Number' }] : [{ value_col: syncedSettings.event_col, label: 'Event Type' }, { value_col: syncedSettings.stdt_col, label: 'Start Date' }, { value_col: syncedSettings.endt_col, label: 'Stop Date' }, { value_col: syncedSettings.seq_col, label: 'Sequence Number' }];
    ...
    syncedSettings.x.format = syncedSettings.time_scale === 'Study Day' ? '1d' : syncedSettings.date_format;
    ...
    syncedSettings.marks[0].tooltip = syncedSettings.time_scale === 'Study Day' ? 'Event: [' + syncedSettings.event_col + ']' + ('\nStart Day: [' + syncedSettings.stdy_col + ']') + ('\nStop Day: [' + syncedSettings.endy_col + ']') : 'Event: [' + syncedSettings.event_col + ']' + ('\nStart Date: [' + syncedSettings.stdt_col + ']') + ('\nStop Date: [' + syncedSettings.endt_col + ']');
    syncedSettings.marks[0].values = syncedSettings.time_scale === 'Study Day' ? { wc_category: [syncedSettings.stdy_col, syncedSettings.endy_col] } : { wc_category: [syncedSettings.stdt_col, syncedSettings.endt_col] };
    ...
    syncedSettings.marks[1].tooltip = syncedSettings.time_scale === 'Study Day' ? 'Event: [' + syncedSettings.event_col + ']' + ('\nStudy Day: [' + syncedSettings.stdy_col + ']') : 'Event: [' + syncedSettings.event_col + ']' + ('\nStudy Date: [' + syncedSettings.stdt_col + ']');
    syncedSettings.marks[1].values = syncedSettings.time_scale === 'Study Day' ? { wc_category: ['DY'] } : { wc_category: ['DT'] };
    ...
    syncedSettings.margin.right = syncedSettings.margin.right ? syncedSettings.margin.right : syncedSettings.time_scale === 'Date' ? 40 : 0;
    ...
    syncedSettings.IDsettings.marks[0].per = [syncedSettings.IDsettings.event_col, syncedSettings.IDsettings.seq_col];
    syncedSettings.IDsettings.marks[1].per = [syncedSettings.IDsettings.event_col, syncedSettings.IDsettings.seq_col, 'wc_value'];
...
function enableDisableControls() {
    ...
    this.controls.wrap.selectAll('.control-group').filter(function (control) {
        return control.value_col !== _this.config.id_col && control.option !== 'event_highlighted' && control.option !== 'time_scale';
...
function onLayout() {
    ...
    this.controls.wrap.selectAll('.control-group').each(function (d) {
        ...
        if (d.value_col === context.config.id_col) context.controls.wrap.insert('div', ':first-child').classed('controls horizontal-rule', true).text('Controls');else if (d.value_col === context.config.site_col) {
...
function IDtimeline(clinicalTimelines) {
    var IDtimeline = webcharts.createChart(clinicalTimelines.rightSide.node(), clinicalTimelines.config.IDsettings);
