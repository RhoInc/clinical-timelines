...
function timeRangeControl(datum) {
    ...
    timeRangeInput.on('change', function (d) {
        ...
        var input = context.config.time_scale === 'date' ? d3.time.format('%Y-%m-%d').parse(this.value) : +this.value;
...
function drawTimeRange() {
    ...
    timeRange[0] <= x_dom[0] && timeRange[1] >= x_dom[1]) // time range fully encompassed x-domain
            ...
            var timeRangeGroup = this.svg.insert('g', '#clinical-timelines .wc-chart .wc-svg .line-supergroup').classed('ct-time-range', true),
                ...
                timeRangeTooltip = timeRangeGroup.append('title').text(this.parent.timelines.config.time_scalePropCased + ' range: ' + timeRangeText.join(' - '));
        }
}
