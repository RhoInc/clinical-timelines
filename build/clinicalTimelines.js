...
function defineStyles() {
    ...
    //Lines
    '#clinical-timelines path.wc-data-mark {' + '    stroke-width: 4;' + '    clip-path: url(#1);' + '    stroke-opacity: 1;' + '}', '#clinical-timelines path.wc-data-mark.highlighted {' + '    stroke-width: 7;' + '}', '#clinical-timelines line.highlight-overlay {' + '    clip-path: url(#1);' + '    stroke-width: 3;' + '    stroke-linecap: round;' + '}',

    //Circles
    '#clinical-timelines circle.wc-data-mark {' + '    stroke-width: 0;' + '    clip-path: url(#1);' + '    fill-opacity: 1;' + '}', '#clinical-timelines circle.wc-data-mark.highlighted {' + '    stroke-opacity: 1;' + '    stroke-width: 2;' + '}',

    //Arrows
    '#clinical-timelines polygon.ongoing-event {' + '    clip-path: url(#1);' + '}', '#clinical-timelines polygon.ongoing-event.highlighted {' + '    stroke-width: 2;' + '}',

    //Reference lines
    '#clinical-timelines .wc-chart .wc-svg title {' + '    white-space: pre;' + '}', '#clinical-timelines > #right-side .wc-chart .wc-svg .visible-reference-line {' + '    stroke: black;' + '    stroke-width: 2px;' + '    stroke-dasharray: 2,2;' + '}', '#clinical-timelines > #right-side .wc-chart .wc-svg .visible-reference-line.hover {' + '    stroke-dasharray: none;' + '}', '#clinical-timelines > #right-side .wc-chart .wc-svg .invisible-reference-line {' + '    stroke: black;' + '    stroke-width: 20px;' + '    stroke-opacity: 0;' + '}', '#clinical-timelines > #right-side .wc-chart .wc-svg .reference-line-label-box {' + '    fill: white;' + '    stroke: black;' + '    stroke-width: black;' + '}',
...
function setDefaultTimeRanges() {
    ...
    //Day range
    this.config.full_day_range = [d3.min(this.initial_data, function (d) {
        return +d[_this.config.stdy_col];
    }), d3.max(this.initial_data, function (d) {
        return +d[_this.config.endy_col];
    })];
    this.config.day_range = this.config.day_range instanceof Array && this.config.day_range.length === 2 && this.config.day_range[0].toString() !== this.config.day_range[1].toString() && this.config.day_range.every(function (day) {
    ...
    //Date range
    this.config.full_date_range = [d3.min(this.initial_data, function (d) {
    ...
    this.config.date_range = this.config.date_range instanceof Array && this.config.date_range.length === 2 && this.config.date_range[0].toString() !== this.config.day_range[1].toString() && this.config.date_range.every(function (date) {
...
function onInit() {
    ...
    this.config.time_range = this.config.time_scale === 'day' ? this.config.day_range : this.config.date_range;
...
function timeRangeControl(datum) {
    var context = this,
        timeRangeContainer = this.controls.wrap.insert('div', '#control-y-sort').datum(datum).classed('control-group ct-control', true).attr('id', 'control-' + datum.option.replace('.', '-')),
        timeRangeInput = timeRangeContainer.append('input').classed('changer', true).property({
        'type': 'date',
        'value': d3.time.format('%Y-%m-%d')(this.config.full_date_range[datum.index])
    }),
    ...
    timeRangeLabelDescription.append('span').classed('span-description', true).text(datum.description);

    //Add event listener to input node.
    timeRangeInput.on('change', function () {
        context.config.x.domain[datum.index] = d3.time.format('%Y-%m-%d').parse(this.value);
        context.draw();
    });
}
...
function drawTimeRange() {
    ...
    var x_dom = this.x_dom.map(function (x) {
        ...
        timeRange = this.parent.clinicalTimelines.config.time_scale === 'day' ? this.parent.clinicalTimelines.config.day_range : this.parent.clinicalTimelines.config.date_range.map(function (dt) {
        return dt.getTime();
    }),
        timeRangeText = this.config.time_scale === 'day' ? this.parent.clinicalTimelines.config.day_range.map(function (dy) {
        return dy.toString();
    ...
    timeRange[0] <= x_dom[0] && timeRange[1] >= x_dom[1]) // time range fully encompassed x-domain
            ...
            var timeRangeGroup = this.svg.insert('g', '#clinical-timelines .wc-chart .wc-svg .line-supergroup').classed('time-range', true),
                ...
                timeRangeTooltip = timeRangeGroup.append('title').text(this.parent.clinicalTimelines.config.time_scale + ' Range: ' + timeRangeText.join(' - '));
        }
}
