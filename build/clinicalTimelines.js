...
function arrayOfVariablesCheck(defaultVariables, userDefinedVariables) {
    var validSetting = userDefinedVariables instanceof Array && userDefinedVariables.length ? d3.merge([defaultVariables, userDefinedVariables.filter(function (item) {
        ...
        var itemObject = item instanceof Object ? Object.assign({}, item) : { value_col: item };
...
function syncRendererSpecificSettings(settings) {
    ...
    settings.id_unitPropCased = settings.id_unit.substring(0, 1).toUpperCase() + settings.id_unit.substring(1).toLowerCase();
    settings.id_unitPlural = /y$/.test(settings.id_unit) ? settings.id_unit.substring(0, settings.id_unit.length - 1) + 'ies' : settings.id_unit + 's';

    //id_characteristics
    var defaultID_characteristics = [{ value_col: settings.id_col, label: settings.id_unitPropCased }];
    settings.id_characteristics = arrayOfVariablesCheck(defaultID_characteristics, settings.id_characteristics);
    ...
    //event_types
    if (!(settings.event_types instanceof Array && settings.event_types.length)) delete settings.event_types;
    ...
    //filters
    var defaultFilters = [{ value_col: settings.id_col, label: settings.id_unitPropCased }, { value_col: settings.event_col, label: 'Event Type' }];
    if (settings.ongo_col) defaultFilters.splice(2, 0, { value_col: settings.ongo_col, label: 'Ongoing?' });
    ...
    //time_scale
    settings.time_scale = ['date', 'day'].indexOf(settings.time_scale.toLowerCase()) > -1 ? settings.time_scale.toLowerCase() : 'date';
    ...
    //  }
    if (settings.date_range && settings.date_ranges === null) settings.date_ranges = [{
        domain: settings.date_range,
        label: 'default'
    }];
    if (settings.date_ranges) settings.date_ranges = settings.date_ranges.filter(function (date_range) {
        var domain = date_range.domain || date_range;
        return domain instanceof Array && domain.length === 2 && domain[0].toString() !== domain[1].toString() && domain.every(function (d) {
            return d instanceof Date || d3.time.format(settings.date_format).parse(d);
        });
    }).map(function (date_range) {
        var domain = (date_range.domain || date_range).map(function (date) {
            return date instanceof Date ? date : d3.time.format(settings.date_format).parse(date);
        });
        var label = date_range.label || domain.map(function (date) {
            return date instanceof Date ? d3.time.format(settings.date_display_format)(date) : date;
        }).join(' - ');
        return {
            domain: domain,
            label: label
    ...
    //  }
    if (settings.day_range && settings.day_ranges === null) settings.day_ranges = [{
        domain: settings.day_range,
        label: 'default'
    }];
    if (settings.day_ranges) settings.day_ranges = settings.day_ranges.filter(function (day_range) {
    ...
    if (settings.reference_lines) {
        if (!(settings.reference_lines instanceof Array)) settings.reference_lines = [settings.reference_lines];

        settings.reference_lines = settings.reference_lines.map(function (reference_line) {
            var referenceLineObject = {};

            //either an object or not
            referenceLineObject.timepoint = reference_line instanceof Object ? reference_line.timepoint : reference_line;

            //either an integer or not
            referenceLineObject.time_scale = Number.isInteger(+referenceLineObject.timepoint) ? 'day' : 'date';

            //label predefined or not
            referenceLineObject.label = reference_line.label ? reference_line.label : referenceLineObject.time_scale.substring(0, 1).toUpperCase() + referenceLineObject.time_scale.substring(1) + ': ' + referenceLineObject.timepoint;

            return referenceLineObject;
        }).filter(function (reference_line) {
            return reference_line.time_scale === 'day' && Number.isInteger(reference_line.timepoint) || reference_line.time_scale === 'date' && d3.time.format(settings.date_format).parse(reference_line.timepoint) instanceof Date;
        });
    ...
    //details
    var defaultDetails = [{ value_col: settings.event_col, label: 'Event Type' }, { value_col: 'stdtdy', label: 'Start Date (Day)' }, { value_col: 'endtdy', label: 'Stop Date (Day)' }, { value_col: settings.seq_col, label: 'Sequence Number' }];
    ...
    if (settings.tooltip_col) defaultDetails.push({ value_col: settings.tooltip_col, label: 'Details' });

    settings.details = arrayOfVariablesCheck(defaultDetails, settings.details);

    //add filters
    settings.filters.filter(function (filter) {
        return filter.value_col !== settings.id_col;
    }) // remove id_col
    .filter(function (filter) {
        return settings.id_characteristics.map(function (id_characteristic) {
            return id_characteristic.value_col;
        }).indexOf(filter.value_col) < 0;
    }) // remove id_characteristics
    .forEach(function (filter) {
        if (settings.details.map(function (detail) {
            return detail.value_col;
        }).indexOf(filter.value_col) === -1) settings.details.push(filter);
    });
}
...
function syncWebchartsSettings(settings) {
    ...
    if (settings.grouping_initial) {
        ...
        settings.y.groupingLabel = settings.groupings[settings.groupings.map(function (grouping) {
            return grouping.value_col;
        }).indexOf(settings.grouping_initial)].label;
    }
...
function syncTimeScaleSettings(settings) {
    settings.time_scalePropCased = settings.time_scale.substring(0, 1).toUpperCase() + settings.time_scale.substring(1);
    ...
    if (settings.time_scale === 'date') {
        ...
        settings.time_function = function (dt) {
            return settings.x_parseFormat.parse(dt) ? settings.x_parseFormat.parse(dt) : new Date(dt);
        };
...
function syncControls(controls, settings) {
    settings.filters.sort(function (a, b) {
        return a.value_col === settings.event_col ? 2 : a.value_col === settings.ongo_col ? 1 : 0;
    }).forEach(function (filter) {
        ...
        //Update ID filter.
        if (filter.value_col === settings.id_col) filter.description = settings.id_unitPropCased + ' view';
...
function defineStyles() {
    ...
    //general
    'html {' + '    overflow: -moz-scrollbars-vertical;' + '    overflow-y: scroll;' + '}', '#clinical-timelines {' + '    display: inline-block;' + '    width: 100%;' + '}',

    //hidden
    '#clinical-timelines .ct-hidden {' + '    display: none !important;' + '}',

    //buttons
    '#clinical-timelines .ct-button {' + '    display: inline-block;' + '    border: 2px solid black !important;' + '    border-radius: 4px !important;' + '    color: #333;' + '    background: #ccc;' + '    cursor: pointer !important;' + '}', '#clinical-timelines .ct-button:hover {' + '    color: #ccc;' + '    background: #333 !important;' + '}', '#clinical-timelines .ct-button.ct-hover {' + '    color: #ccc;' + '    background: #333 !important;' + '}',

    //info icons
    '#clinical-timelines .ct-info-icon {' + '    font-size: 16px;' + '    font-weight: bold;' + '    color: blue;' + '    cursor: help;' + '}',

    /***--------------------------------------------------------------------------------------\
      Left and right columns
    \--------------------------------------------------------------------------------------***/

    '#clinical-timelines .ct-column {' + '    display: inline-block;' + '}', '#clinical-timelines #ct-left-column {' + '    width: 24%;' + '    float: left;' + '}', '#clinical-timelines #ct-right-column {' + '    width: 75%;' + '    float: right;' + '}', '#clinical-timelines .ct-column > * {' + '    width: 100%;' + '    vertical-align: top;' + '    display: inline-block;' + '    margin-bottom: 10px;' + '    border: 1px solid #eee;' + '}', '#clinical-timelines .ct-column > * > * {' + '    margin: 10px;' + '}',
    ...
    //Details
    '#clinical-timelines #ct-left-column .ct-details {' + '    font-size: 90%;' + '    text-align: right;' + '}', '#clinical-timelines #ct-left-column .ct-details .ct-button {' + '    float: left;' + '    font-size: 110%;' + '    font-weight: bold;' + '    padding: 1px 5px !important;' + '}', '#clinical-timelines #ct-left-column .ct-details .ct-button a {' + '    color: blue;' + '    text-decoration: none;' + '}', '#clinical-timelines #ct-left-column .ct-details .ct-button:hover a {' + '    color: #ccc;' + '}', '#clinical-timelines #ct-left-column .ct-details .ct-stats,' + '#clinical-timelines #ct-left-column .ct-details .ct-characteristic span {' + '    font-weight: bold;' + '}', '#clinical-timelines #ct-left-column .ct-details .ct-stats.ct-sample {' + '    color: green;' + '}', '#clinical-timelines #ct-left-column .ct-details .ct-stats.ct-sample-inside-time-range {' + '    color: blue;' + '}', '#clinical-timelines #ct-left-column .ct-details .ct-stats.ct-sample-outside-time-range {' + '    color: red;' + '}',

    //Controls
    '#clinical-timelines #ct-left-column #ct-controls .ct-button {' + '    float: right;' + '    font-size: 110%;' + '    font-weight: bold;' + '    padding: 1px 5px !important;' + '    color: blue;' + '    text-decoration: none;' + '}', '#clinical-timelines #ct-left-column #ct-controls .ct-button:hover {' + '    color: #ccc;' + '}', '#clinical-timelines #ct-left-column .wc-controls {' + '    margin-bottom: 0;' + '    clear: left;' + '}', '#clinical-timelines #ct-left-column .wc-controls .control-group {' + '    margin: 0 0 5px 0;' + '    display: block;' + '    float: right;' + '    clear: both;' + '    width: 100%;' + '}', '#clinical-timelines #ct-left-column .wc-controls .ct-horizontal-rule {' + '    width: 100%;' + '    display: inline-block;' + '    margin-bottom: 5px;' + '    font-size: 150%;' + '    border-bottom: 1px solid black;' + '}', '#clinical-timelines #ct-left-column .wc-controls .control-group.ct-ID {' + '    display: inline-block;' + '}', '#clinical-timelines #ct-left-column .wc-controls .control-group > * {' + '    display: inline-block;' + '    vertical-align: top;' + '    float: right;' + '}', '#clinical-timelines #ct-left-column .wc-controls .control-group .span-description {' + '    font-size: 90%;' + '}', '#clinical-timelines #ct-left-column .wc-controls .control-group .changer {' + '    margin-left: 5px;' + '    width: 50%;' + '    clear: right;' + '    box-sizing: border-box;' + '}',

    //Reference Tables
    '#clinical-timelines #ct-left-column .ct-reference-line-table-header {' + '    text-align: center;' + '    border-bottom: 1px solid black;' + '    padding-bottom: 5px;' + '}', '#clinical-timelines #ct-left-column .ct-reference-line-table-body table {' + '    width: 100%;' + '    display: table;' + '}', '#clinical-timelines #ct-left-column .ct-reference-line-table-body th,' + '#clinical-timelines #ct-left-column .ct-reference-line-table-body td {' + '    text-align: left;' + '}', '#clinical-timelines #ct-left-column .ct-higher-level {' + '    border-bottom: 1px dotted lightgray;' + '    font-weight: bold;' + '    font-size: 14px;' + '}', '#clinical-timelines #ct-left-column .ct-lower-level {' + '    font-size: 12px;' + '}', '#clinical-timelines #ct-left-column .ct-lower-level.ct-indent {' + '    padding-left: 5%;' + '}',
    ...
    //Select All button
    '#clinical-timelines #ct-right-column #ct-timelines .ct-select-all {' + '    position: relative;' + '    z-index: 2;' + '    float: right;' + '    font-size: .9em;' + '    font-weight: bold;' + '    padding: 2px 5px !important;' + '    color: blue;' + '    text-decoration: none;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .ct-select-all:hover {' + '    color: #ccc;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .wc-chart {' + '    position: relative;' + '    z-index: 1;' + '}',

    //Legend
    '#clinical-timelines #ct-right-column #ct-timelines .legend {' + '    display: flex !important;' + '    justify-content: center;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .legend-title {' + '    border-radius: 4px;' + '    padding: 5px 7px 3px 4px;' + '    border: 2px solid white;' + '    margin-right: .25em !important;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .legend-item {' + '    cursor: pointer;' + '    float: left;' + '    border-radius: 4px;' + '    padding: 4px 7px 3px 4px;' + '    border: 2px solid white;' + '    margin-right: .25em !important;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .legend-item:not(.ct-highlighted) {' + '    border: none !important;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .legend-item:not(.ct-selected) {' + '    background: white;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .legend-color-block circle {' + '    cx: .55em !important;' + '    cy: .55em !important;' + '    r: .4em !important;' + '    stroke: black;' + '    stroke-width: 1;' + '}',

    //Y-axis
    '#clinical-timelines #ct-right-column #ct-timelines .wc-chart .wc-svg .y.axis .tick {' + '    cursor: pointer;' + '    fill: blue;' + '    text-decoration: none;' + '    font-weight: bolder;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .wc-chart .wc-svg .y.axis .tick:hover {' + '    text-decoration: underline;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .wc-chart .wc-svg .y.axis .tick rect.ct-stripe {' + '    stroke: #aaa;' + '    stroke-width: 1;' + '    fill: none;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .wc-chart .wc-svg .y.axis .tick:nth-child(even) rect.ct-stripe {' + '    fill: #eee;' + '}',

    //Grouping
    '#clinical-timelines #ct-right-column #ct-timelines .wc-chart .wc-svg .ct-grouping .ct-boundary {' + '    stroke: black;' + '    stroke-width: 2;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .wc-chart .wc-svg .ct-grouping .ct-annotation {' + '    font-size: 150%;' + '    font-weight: normal;' + '    text-anchor: start;' + '}', '#clinical-timelines #ct-right-column #ct-timelines .wc-chart .wc-svg .ct-grouping.ct-vertical .ct-annotation {' + '    writing-mode: tb-rl;' + '}',

    //Lines
    '#clinical-timelines path.wc-data-mark {' + ('    stroke-width: ' + line.attributes['stroke-width'] + ';') + '    stroke-opacity: 1;' + '}', '#clinical-timelines path.wc-data-mark.ct-highlighted {' + ('    stroke-width: ' + line.attributes['stroke-width'] * 1.5 + ';') + '}', '#clinical-timelines line.ct-highlight-overlay {' + '    clip-path: url(#1);' + ('    stroke-width: ' + line.attributes['stroke-width'] / 2 + ';') + '    stroke-linecap: round;' + '}',

    //Circles
    '#clinical-timelines circle.wc-data-mark:not(.ct-highlighted) {' + '    stroke-width: 1;' + '    stroke: black;' + '    fill-opacity: 1;' + '}', '#clinical-timelines circle.wc-data-mark.ct-highlighted {' + '    stroke-opacity: 1;' + '    fill-opacity: 1;' + ('    stroke-width: ' + circle.attributes['stroke-width'] + ';') + '}',

    //Symbols
    '#clinical-timelines .ct-custom-mark {' + '    stroke-width: 1;' + '    stroke: black;' + '}', '#clinical-timelines .ct-custom-mark.ct-highlighted {' + '    stroke-opacity: 1;' + '    stroke-width: 2;' + '}',

    //Arrows
    '#clinical-timelines polygon.ct-ongoing-event {' + '    clip-path: url(#1);' + '}', '#clinical-timelines polygon.ct-ongoing-event.ct-highlighted {' + ('    stroke-width: ' + line.attributes['stroke-width'] / 3 + ';') + '}',

    //Reference lines
    '#clinical-timelines #ct-right-column .wc-chart .wc-svg .ct-visible-reference-line {' + '    stroke: black;' + '    stroke-width: 1;' + '    stroke-dasharray: 2,2;' + '}', '#clinical-timelines #ct-right-column .wc-chart .wc-svg .ct-visible-reference-line.ct-hover {' + '    stroke-dasharray: none;' + '}', '#clinical-timelines #ct-right-column .wc-chart .wc-svg .ct-hover-line {' + '    cursor: pointer;' + '    stroke: black;' + '    stroke-width: 20;' + '    stroke-opacity: 0;' + '}', '#clinical-timelines #ct-right-column .wc-chart .wc-svg .ct-reference-line-text {' + '    font-weight: bold;' + '    font-size: 24px;' + '}',

    //ID timeline
    '#clinical-timelines #ct-right-column #ct-ID-timeline .wc-chart .wc-svg title {' + '    white-space: pre;' + '}', '#clinical-timelines #ct-right-column #ct-ID-timeline .wc-small-multiples .wc-chart {' + '    width: 100%;' + '    padding: 0;' + '    border-top: 1px solid black;' + '}', '#clinical-timelines #ct-right-column #ct-ID-timeline .wc-small-multiples .wc-chart > * {' + '    display: inline-block;' + '}', '#clinical-timelines #ct-right-column #ct-ID-timeline .wc-small-multiples .wc-chart .wc-svg {' + '    float: left;' + '    width: 75%;' + '}', '#clinical-timelines #ct-right-column #ct-ID-timeline .wc-small-multiples .wc-chart .wc-chart-title {' + '    float: right;' + '    text-align: left;' + '    font-size: 150%;' + '    font-weight: normal;' + '    width: 24%;' + '}', '#clinical-timelines #ct-right-column #ct-ID-timeline .wc-small-multiples .wc-chart .wc-svg .ct-time-range {' + '    opacity: .1;' + '}',

    //Listing
    '#clinical-timelines #ct-right-column #ct-listing .wc-chart.wc-table table {' + '    display: table;' + '    width: 100%;' + '}'];
...
function csv(data, headers, columns) {
    ...
    data.forEach(function (d, i) {
        //Add headers to CSV array.
        if (i === 0) CSVarray.push(headers.map(function (header) {
    ...
    var CSV = new Blob([CSVarray.join('\n')], { type: 'text/csv;charset=utf-8;' }),
        fileName = 'ClinicalTimelinesData_' + d3.time.format('%Y-%m-%dT%H-%M-%S')(new Date()) + '.csv',
...
function exportData() {
    ...
    var data = this.timelines.wideDataInsideTimeRange.filter(function (d) {
        var st = _this.timelines.config.time_function(d[_this.timelines.config.st_col]),
            ...
            stInsideTimeRange = _this.timelines.config.x.domain[0] <= st && st <= _this.timelines.config.x.domain[1],
            // start is within the time range
        enInsideTimeRange = _this.timelines.config.x.domain[0] <= en && en <= _this.timelines.config.x.domain[1],
            // end is within the time range
        surroundingTimeRange = _this.timelines.config.x.domain[0] > st && en > _this.timelines.config.x.domain[1]; // start is prior to time range and end is after time range
    ...
    if (this.settings.transpose_data) {
        ...
        data = transposed;
        columns = this.settings.id_characteristics.map(function (id_characteristic) {
            return id_characteristic.label;
        }).concat([this.settings.seq_col]).concat(this.timelines.currentEventTypes.concat(this.timelines.currentEventTypes).sort().map(function (eventType, i) {
            return eventType + (i % 2 ? ' End' : ' Start');
        }));
        headers = columns;
    } else {
        //Otherwise export one record per ID per event.
        var variables = this.settings.id_characteristics.concat(this.settings.details.filter(function (detail) {
            return _this.settings.id_characteristics.map(function (id_characteristic) {
                return id_characteristic.value_col;
    ...
    if (typeof XLSX === 'undefined') csv.call(this, data, headers, columns);else xlsx.call(this, data, headers, columns);
}
...
function hideTimeRangeControl() {
    ...
    this.controls.wrap.selectAll('.control-group').filter(function (d) {
        ...
        return _this.config.time_scale === 'date' && d.option === 'day_time_range' || _this.config.time_scale === 'day' && d.option === 'date_time_range';
    });
}
...
function enableDisableControls() {
    ...
    this.controls.wrap.selectAll('.control-group').filter(function (control) {
        return control.value_col !== _this.config.id_col && control.option !== 'event_highlighted' && control.option !== 'time_scale' && control.value_col !== _this.config.event_col;
    ...
    this.clinicalTimelines.containers.leftColumn.selectAll('.ct-reference-line-table-container').classed('ct-hidden', !!this.selected_id);
...
function updateIDfilter() {
    ...
    })[0].val = this.selected_id || 'All';

    //Bring focus to the ID dropdown.
    IDfilter.style({
        'font-weight': 'bold'
    }).transition().delay(1000).style({
        'font-weight': 'normal'
    }).select('select').node().focus();
}
...
function drawIDtimeline() {
    ...
    if (this.config.id_characteristics) {
        ...
        this.clinicalTimelines.containers.IDdetails.selectAll('.ct-ID-detail').each(function (d) {
            d3.select(this).select('span').text(id_characteristics[d.value_col]);
        });
    }
    ...
    if (this.config.id_urls) {
        ...
        this.clinicalTimelines.containers.IDdetails.selectAll('.ct-ID-URL').each(function (d) {
            d3.select(this).classed('ct-hidden', !id_urls[d.value_col]).select('a').attr({
    ...
    this.clinicalTimelines.containers.IDtimeline.select('div').selectAll('*').remove();
    webcharts.multiply(this.IDtimeline, longIDdata.filter(function (d) {
        return _this.currentEventTypes !== 'All' ? _this.currentEventTypes.indexOf(d[_this.config.event_col]) > -1 : true;
    }), this.config.event_col, this.config.color_dom, clinicalTimelines.test);

    //Draw ID detail listing.
    this.clinicalTimelines.containers.listing.classed('ct-hidden', false);
    this.clinicalTimelines.containers.listing.select('div').selectAll('*').remove();
    this.listing.init(wideIDdata.filter(function (d) {
        return _this.currentEventTypes !== 'All' ? _this.currentEventTypes.indexOf(d[_this.config.event_col]) > -1 : true;
...
function selectAllEventTypes() {
    ...
    //Update event type filter.
    this.timelines.controls.wrap.selectAll('.control-group').filter(function (d) {
        return d.value_col === _this.settings.event_col;
    }).selectAll('.changer option').property('selected', true);
    ...
    //Draw chart.
    if (this.selected_id) drawIDtimeline.call(this.timelines);else this.timelines.draw();
}

function defineLayout() {
    var _this = this;

    this.containers.main = d3.select(this.element).append('div').attr('id', 'clinical-timelines');

    /**-------------------------------------------------------------------------------------------\
      Left column
    \-------------------------------------------------------------------------------------------**/

    this.containers.leftColumn = this.containers.main.append('div').classed('ct-column', true).attr('id', 'ct-left-column');
    ...
    this.containers.populationDetails = this.containers.details.append('div').classed('ct-details', true).attr('id', 'ct-population-details');

    //Add button to export timelines data.
    this.containers.exportButton = this.containers.populationDetails.append('div').classed('ct-button', true).attr('id', 'ct-export-button').append('a').html('&dArr; Export Data').attr('title', 'Save current data as an Excel spreadsheet.').on('click', function () {
        exportData.call(_this);
    });

    //Add container for population details given current filters.
    this.containers.popCurrentFilters = this.containers.populationDetails.append('div').classed('ct-characteristic', true).attr('id', 'ct-current-filters');

    //Add container for population details inside time range.
    this.containers.popInsideTimeRange = this.containers.populationDetails.append('div').classed('ct-characteristic', true).attr('id', 'ct-inside-time-range');

    //Add container for population details inside time range.
    this.containers.popOutsideTimeRange = this.containers.populationDetails.append('div').classed('ct-characteristic', true).attr('id', 'ct-outside-time-range');

    /****---------------------------------------------------------------------------------\
      ID details
    \---------------------------------------------------------------------------------****/

    this.containers.IDdetails = this.containers.details.append('div').classed('ct-details ct-hidden', true).attr('id', 'ct-ID-details');

    //Add button to return from ID timeline to timelines.
    this.containers.backButton = this.containers.IDdetails.append('div').classed('ct-button', true).attr('id', 'ct-back-button').append('a').html('&lArr; Back to Timelines').attr('title', 'Return to the view of all ' + this.settings.id_unitPlural + '.').on('click', function () {
    ...
    this.containers.controls.append('div').classed('ct-button ct-reset', true).text('Reset Controls').attr('title', 'Reset controls to initial settings.').on('click', function () {
    ...
    this.containers.rightColumn = this.containers.main.append('div').classed('ct-column', true).attr('id', 'ct-right-column');

    /***--------------------------------------------------------------------------------------\
      Timelines
    \--------------------------------------------------------------------------------------***/

    this.containers.timelines = this.containers.rightColumn.append('div').attr('id', 'ct-timelines');

    //Add button that selects all event types.
    this.containers.timelines.append('div').classed('ct-button ct-select-all', true).html('&#10003;').attr('title', 'Select all event types.').on('click', function () {
    ...
    this.containers.IDtimeline = this.containers.rightColumn.append('div').classed('ct-hidden', true).attr('id', 'ct-ID-timeline');

    /***--------------------------------------------------------------------------------------\
      Listing
    \--------------------------------------------------------------------------------------***/

    this.containers.listing = this.containers.rightColumn.append('div').classed('ct-hidden', true).attr('id', 'ct-listing');
}
...
function removeInvalidData() {
    ...
    this.initial_data = this.raw_data.filter(function (d) {
        return !/^\s*$/.test(d[_this.config.id_col]) && !/^\s*$/.test(d[_this.config.event_col]);
    });

    //Warn user of removed records.
    if (this.initial_data.length < this.raw_data.length) console.warn(this.raw_data.length - this.initial_data.length + " records have been removed due to missing identifiers or event types.");
}
...
function standardizeTimeVariables() {
    ...
    this.initial_data.forEach(function (d, i) {
        ...
        if (has_stdt) {
            if (!d3.time.format(_this.config.date_format).parse(d[_this.config.stdt_col])) d[_this.config.stdt_col] = '';
        }
        if (has_endt) {
            if (!d3.time.format(_this.config.date_format).parse(d[_this.config.endt_col])) d[_this.config.endt_col] = d[_this.config.stdt_col];
        }
        if (has_stdy) {
            if (!/^ *\d+ *$/.test(d[_this.config.stdy_col])) d[_this.config.stdy_col] = '';
        }
        if (has_endy) {
            if (!/^ *\d+ *$/.test(d[_this.config.endy_col])) d[_this.config.endy_col] = d[_this.config.stdy_col];
        }

        //Concatenate date and day values for listing.
        d.stdtdy = has_stdt && has_stdy && d[_this.config.stdy_col] !== '' ? d[_this.config.stdt_col] + ' (' + d[_this.config.stdy_col] + ')' : d[_this.config.stdt_col] || d[_this.config.stdy_col];
        d.endtdy = has_endt && has_endy && d[_this.config.endy_col] !== '' ? d[_this.config.endt_col] + ' (' + d[_this.config.endy_col] + ')' : d[_this.config.endt_col] || d[_this.config.endy_col];
    });
}
...
function handleEventTypes() {
    ...
    this.allEventTypes = d3.set(this.initial_data.map(function (d) {
    ...
    this.config.color_dom = this.currentEventTypes.concat(this.allEventTypes.filter(function (eventType) {
...
function defineFullTimeRanges() {
    ...
    //full domain
    this.full_date_range = [d3.min(this.initial_data, function (d) {
    ...
    this.full_date_time_range = this.full_date_range.map(function (d) {
...
function syncTimeRanges() {
    if (this.config.time_scale === 'date') this.time_range = this.date_range.slice();else this.time_range = this.day_range.slice();
}
...
function checkTimeScales() {
    ...
    this.controls.config.inputs = this.controls.config.inputs.filter(function (input) {
        if (input.description !== 'X-axis scale') return true;else {
            ...
            if (!anyDates && !anyDays) {
                var errorText = 'The data either contain neither ' + _this.config.stdt_col + ' nor ' + _this.config.stdy_col + ' or both ' + _this.config.stdt_col + ' and ' + _this.config.stdy_col + ' contain no valid values.  Please update the settings object to match the variables in the data or clean the data.';
                _this.wrap.append('div').style('color', 'red').html(errorText);
                throw new Error(errorText);
            } else if (!anyDates && _this.config.time_scale === 'date') {
                console.warn('The data either do not contain a variable named ' + _this.config.stdt_col + ' or ' + _this.config.stdt_col + ' contains no valid values.  Please update the settings object to match the variable in the data or clean the data.');
                ...
                console.warn('The data either do not contain a variable named ' + _this.config.stdy_col + ' or ' + _this.config.stdy_col + ' contains no valid values.  Please update the settings object to match the variable in the data or clean the data.');
...
function checkOtherControls() {
    ...
    this.controls.config.inputs.filter(function (input) {
        ...
        if (input.description === 'Event highlighting') input.values = _this.config.color_dom.slice();else if (input.description === 'Y-axis grouping') input.values = _this.config.groupings.map(function (grouping) {
            return grouping.value_col;
        });
...
function checkFilters() {
    ...
    this.controls.config.inputs = this.controls.config.inputs.filter(function (input) {
        if (input.type !== 'subsetter') return true;else if (!_this.initial_data[0].hasOwnProperty(input.value_col)) {
            console.warn(input.value_col + ' filter removed because the variable does not exist.');

            return false;
        } else {
            var levels = d3.set(_this.initial_data.map(function (d) {
                return d[input.value_col];
            })).values();

            if (levels.length < 2) {
                console.warn(input.value_col + ' filter removed because the variable has only one level.');
            }
...
function cleanData() {
    ...
    if (this.wide_data.length < this.initial_data.length) {
        if (this.config.time_scale === 'day') console.warn(this.initial_data.length - this.wide_data.length + ' records have been removed due to missing or invalid day variable values.');else if (this.config.time_scale === 'date') console.warn(this.initial_data.length - this.wide_data.length + ' records have been removed due to missing or invalid date variable values that do not match settings.date_format (' + this.config.date_format + ')');
    }
}
...
function defineData() {
    ...
    //Capture timepoints.
    var timepoints = this.wide_data.filter(function (d) {
    ...
    var timeIntervals = lengthenRaw(this.wide_data.filter(function (d) {
        return d[_this.config.st_col] !== d[_this.config.en_col];
...
function IDdetails() {
    ...
    this.clinicalTimelines.containers.IDdetails.selectAll('div.ct-characteristic.ct-ID-detail').data(this.config.id_characteristics).enter().append('div').classed('ct-characteristic ct-ID-detail', true).html(function (d) {
        return d.label + ': <span id = \'' + d.value_col + '\'></span>';
    });

    //Add ID characteristics.
    if (Array.isArray(this.config.id_urls) && this.config.id_urls.length) {
        this.clinicalTimelines.containers.IDdetails.selectAll('div.ct-characteristic.ct-ID-URL').data(this.config.id_urls).enter().append('div').classed('ct-characteristic ct-ID-URL', true).html(function (d) {
...
function controlGroupLayout() {
    ...
    this.controls.wrap.selectAll('.control-group').each(function (d) {
        ...
        if (d.value_col === context.config.id_col) context.controls.wrap.insert('div', ':first-child').classed('ct-controls ct-horizontal-rule', true).text('Controls');else if (context.config.filters.some(function (filter) {
            return [context.config.id_col, context.config.event_col, context.config.ongo_col].indexOf(filter.value_col) < 0;
        }) && context.config.groupings.length && d.option === 'y.grouping' || !context.config.groupings.length && d.option === 'y.sort') {
            var filterRule = context.controls.wrap.append('div').classed('ct-horizontal-rule ct-filters ct-ID-filters', true).text(context.config.id_unitPropCased + ' Filters ');
            filterRule.append('span').classed('ct-info-icon', true).html('&#x24d8;').attr('title', 'These filters control the set of ' + context.config.id_unitPlural + ' displayed on the y-axis.');
            context.controls.wrap.node().insertBefore(filterRule.node(), this.nextSibling);
        } else if (d.value_col === context.config.ongo_col || d.value_col === context.config.event_col && !context.config.ongo_col) {
            var _filterRule = context.controls.wrap.append('div').classed('ct-horizontal-rule ct-filters ct-event-filters', true).text('Event Filters ');
            _filterRule.append('span').classed('ct-info-icon', true).html('&#x24d8;').attr('title', 'These filters control the set of events visible in the chart.');
...
function timeRange(dropdown, d) {
    var label = d3.select(dropdown).selectAll('option').filter(function () {
...
function augmentOtherControls() {
    var context = this;
    var controls = this.controls.wrap.selectAll('.control-group').filter(function (d) {
        return d.type !== 'subsetter';
    }).classed('ct-control', true).attr('id', function (d) {
        return 'control-' + d.option.replace('.', '-');
    });

    //Relabel Y-axis sort options and remove illogical Y-axis grouping options.
    controls.filter(function (d) {
        ...
        if (d.description === 'Y-axis sort')
            ...
            options.property('label', function (di) {
                return d.relabels[d.values.filter(function (dii) {
                    return dii !== 'None';
                }).indexOf(di)];
            });else if (d.description === 'Y-axis grouping')
            // Add variable labels to Y-axis grouping options.
            options.property('label', function (di) {
                return di !== 'None' ? context.config.groupings[context.config.groupings.map(function (dii) {
                    return dii.value_col;
                }).indexOf(di)].label : 'None';
            });
    });
...
function onChange(input, d) {
    ...
    //User input.
    var inputValue = this.config.time_scale === 'date' ? d3.time.format('%Y-%m-%d').parse(input.value) : +input.value;

    //handle invalid inputs
    if (d.index === 0 && inputValue >= this[time_range][1]) inputValue = this.config.time_scale === 'date' ? new Date(this[time_range][1].getTime() - increment) : this[time_range][1] - increment;else if (d.index === 1 && inputValue <= this[time_range][0]) inputValue = this.config.time_scale === 'date' ? new Date(this[time_range][0].getTime() + increment) : inputValue = this[time_range][0] + increment;
    ...
    }).selectAll('option').property('selected', function () {
...
function timeRangeControl(datum) {
    var context = this,
        timeRangeContainer = this.controls.wrap.insert('div', '#control-y-sort').datum(datum).classed('control-group ct-time-range ct-control', true).attr('id', function (d) {
        return 'control-' + d.option.replace(/\./g, '-');
    }),
        timeRangeInput = timeRangeContainer.append('input').classed('changer', true),
        timeRangeLabelDescription = timeRangeContainer.append('div').classed('ct-label-description', true);

    //Add control label and span description nodes.
    timeRangeLabelDescription.append('span').classed('control-label', true);
    timeRangeLabelDescription.append('span').classed('span-description', true).text(function (d) {
...
function IDchange(select$$1) {
    ...
    this.selected_id = d3.select(select$$1).select('option:checked').text();
    this.filters.filter(function (filter) {
        return filter.col === _this.config.id_col;
...
function eventTypeChange(select$$1) {
    ...
    this.currentEventTypes = d3.select(select$$1).selectAll('select option:checked').pop().map(function (d) {
...
function augmentFilters() {
    ...
    var controls = this.controls.wrap.selectAll('.control-group').filter(function (d) {
        return d.type === 'subsetter';
    }).classed('ct-filter', true).attr('id', function (d) {
    ...
    eventTypeFilter.selectAll('select.changer option').property('selected', function (di) {
        return context.currentEventTypes instanceof Array ? context.currentEventTypes.indexOf(di) > -1 : true;
    });
...
function topXaxis() {
    this.svg.append('g').classed('x-top axis linear', true).append('text').classed('axis-title top', true);
}
    ...
    this.filtered_long_data = this.long_data.filter(function (d) {
        ...
        _this.filters.forEach(function (di) {
            if (filtered === false && di.val !== 'All') {
                filtered = di.val instanceof Array ? di.val.indexOf(d[di.col]) === -1 : di.val !== d[di.col];
            }
        });
...
function updatePopulationDetails() {
    this.clinicalTimelines.containers.popCurrentFilters.classed('ct-hidden', this.populationDetails.nOutsideTimeRange).html('<span class = \'ct-stats ct-sample\'>' + this.populationDetails.n + '</span> of <span class = \'ct-stats\'>' + this.populationDetails.N + '</span> ' + (this.populationDetails.N > 1 ? this.config.id_unitPlural : this.config.id_unit) + ' (<span class = \'ct-stats\'>' + d3.format('%')(this.populationDetails.rate) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' have data that meet the current filter criteria.\'>&#9432;</span>');
    this.clinicalTimelines.containers.popInsideTimeRange.html(this.populationDetails.nInsideTimeRange < this.populationDetails.n ? '<span class = \'ct-stats ct-sample-inside-time-range\'>' + this.populationDetails.nInsideTimeRange + '</span> of <span class = \'ct-stats ct-sample\'>' + this.populationDetails.n + '</span> displayed (<span class = \'ct-stats\'>' + d3.format('%')(this.populationDetails.rateInsideTimeRange) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' have events that occur in the current time range.\'>&#9432;</span>' : '');
    this.clinicalTimelines.containers.popOutsideTimeRange.html(this.populationDetails.nOutsideTimeRange ? '<span class = \'ct-stats ct-sample-outside-time-range\'>' + this.populationDetails.nOutsideTimeRange + '</span> of <span class = \'ct-stats ct-sample\'>' + this.populationDetails.n + '</span> hidden (<span class = \'ct-stats\'>' + d3.format('%')(this.populationDetails.rateOutsideTimeRange) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' do not have events that occur in the current time range.\'>&#9432;</span>' : '');
}
...
function definePopulationDetails() {
    ...
    this.populationDetails.sample = d3.set(this.filtered_wide_data.map(function (d) {
        return d[_this.config.id_col];
        ...
        IDobject.dataInsideTimeRange = IDobject.data.filter(function (d) {
            var st = _this.config.time_function(d[_this.config.st_col]),
                ...
                stInsideTimeRange = _this.config.x.domain[0] <= st && st <= _this.config.x.domain[1],
                // start is within the time range
            enInsideTimeRange = _this.config.x.domain[0] <= en && en <= _this.config.x.domain[1],
                // end is within the time range
            surroundingTimeRange = _this.config.x.domain[0] > st && en > _this.config.x.domain[1]; // start is prior to time range and end is after time range
    ...
    this.populationDetails.sampleInsideTimeRange = this.populationDetails.sample.filter(function (d) {
    ...
    this.populationDetails.rateInsideTimeRange = this.populationDetails.nInsideTimeRange / this.populationDetails.n;

    //Define sample given current filters of IDs without an event inside the current time range.
    this.populationDetails.sampleOutsideTimeRange = this.populationDetails.sample.filter(function (d) {
        return !d.dataInsideTimeRange.length;
    }).map(function (d) {
        return d.ID;
    });
    this.populationDetails.nOutsideTimeRange = this.populationDetails.sampleOutsideTimeRange.length;
    this.populationDetails.rateOutsideTimeRange = this.populationDetails.nOutsideTimeRange / this.populationDetails.n;
...
function defineDataInsideTimeRange() {
    ...
    this.longDataInsideTimeRange = this.filtered_long_data.filter(function (d) {
        return _this.populationDetails.sampleInsideTimeRange.indexOf(d[_this.config.id_col]) > -1;
    });
...
function defineGroupingData() {
    ...
    if (this.config.y.grouping) {
        ...
        this.groupings = d3.set(this.longDataInsideTimeRange.map(function (d) {
            ...
            if (_this.config.grouping_direction === 'horizontal') {
                ...
                for (var variable in _this.raw_data[0]) {
                    if ([_this.config.id_col, _this.config.event_col, _this.config.seq_col, _this.config.y.grouping].indexOf(variable) === -1) groupingObject[variable] = '';else if (variable === _this.config.id_col) groupingObject[_this.config.id_col] = d;else if (variable === _this.config.event_col) groupingObject[_this.config.event_col] = 'Grouping';else if (variable === _this.config.seq_col) groupingObject[_this.config.seq_col] = '1';else if (variable === _this.config.y.grouping) groupingObject[_this.config.y.grouping] = d;
                }
...
function sortYdomain() {
    ...
    if (this.config.y.sort === 'earliest') {
        if (this.config.y.grouping) {
            ...
            }).entries(this.longDataInsideTimeRange).sort(function (a, b) {
                var aGrouping = a.key.split('|')[0],
                    bGrouping = b.key.split('|')[0],
                    earliestEventSort = a.values > b.values ? -2 : a.values < b.values ? 2 : a.key > b.key ? -1 : 1;

                return aGrouping > bGrouping ? -1 : aGrouping < bGrouping ? 1 : earliestEventSort;
            ...
            nestedData.forEach(function (d) {
                ...
                _this.groupings.filter(function (grouping) {
                    return grouping.key === split[0];
                }).pop().IDs.push(split[1]);
            });
            ...
            this.config.y.domain = d3.nest().key(function (d) {
                return d[_this.config.id_col];
            }).rollup(function (d) {
                return d3.min(d, function (di) {
                    return _this.config.time_function(di[_this.config.st_col]);
                ...
                var earliestEventSort = a.values > b.values ? -2 : a.values < b.values ? 2 : a.key > b.key ? -1 : 1;
        ...
        if (this.config.y.grouping) {
            ...
            this.config.y.domain = d3.set(this.longDataInsideTimeRange.map(function (d) {
                return d[_this.config.id_col];
            ...
            this.config.y.domain.forEach(function (d) {
                _this.groupings.filter(function (grouping) {
                    return grouping.key === _this.raw_data.filter(function (di) {
            ...
            this.config.y.domain = this.populationDetails.sampleInsideTimeRange.sort(function (a, b) {
...
function legendFilter() {
    ...
    eventTypes = eventTypeControl.selectAll('.changer option').sort(function (a, b) {
        ...
        // event type options
    legendItems = this.wrap.selectAll('.legend-item').classed('ct-button', true).classed('ct-selected', function (d) {
        return eventTypeFilter.val instanceof Array ? eventTypeFilter.val.indexOf(d.label) > -1 : true;
    }).classed('ct-hidden', function (d) {
        return d.label === 'None';
    ...
    legendItems.on('click', function (d) {
        ...
        eventTypes.property('selected', false).filter(function (d) {
        ...
        if (context.selected_id) drawIDtimeline.call(context);else context.draw();
    });
}

function drawTopXaxis() {
    var topXaxis = d3.svg.axis().scale(this.x).orient('top').ticks(this.xAxis.ticks()[0]).tickFormat(this.config.x_displayFormat).innerTickSize(this.xAxis.innerTickSize()).outerTickSize(this.xAxis.outerTickSize()),
        topXaxisSelection = this.svg.select('g.x-top.axis').attr('class', 'x-top axis linear');
    topXaxisSelection.call(topXaxis);
    topXaxisSelection.select('text.axis-title.top').attr({
...
function addTooltip() {
    ...
    this.svg.on('mousemove', function () {
        context.tooltip.text('' + context.config.x_displayFormat(context.x.invert(d3.mouse(this)[0])));
    });
}
...
function tickClick() {
    ...
    this.svg.selectAll('.y.axis .tick text').each(function (d) {
        ...
        if (/^-g\d+-/.test(d)) tick.remove();else {
            var tooltip = context.config.id_tooltip_col ? context.initial_data.find(function (di) {
                return di[context.config.id_col] === d;
            })[context.config.id_tooltip_col] : context.config.id_unitPropCased + ': ' + d;
            tick.append('title').classed('ct-y-tooltip', true).text(tooltip);
        }
    }).on('click', function (d) {
        _this.selected_id = d;
        ...
        ticks.each(function (d, i) {
            ...
            if (!/^-g\d+-/.test(d)) {
                context.config.id_urls.forEach(function (id_url, j) {
                    ...
                    var link = tick.append('text').classed('ct-url', true).classed('ct-hidden', !url).attr({
                        dx: -((j + 1) * 21),
                        dy: context.y.rangeBand() - 7,
                        fill: 'blue',
                        cursor: 'pointer'
                    }).text(id_url.text).on('click', function () {
                        window.open(url);
                    });
...
function horizontally() {
    ...
    this.groupings.sort(function (a, b) {
        ...
        if (d.IDs.length) {
            var nIDs = d.IDs.length,
                ...
                y1 = _this.y(firstID),
                y2 = _this.y(d.IDs[0]),
                g = _this.svg.append('g').classed('ct-grouping ct-horizontal', true).attr('id', d.key.replace(/ /g, '-')),
                annotation = g.append('text').classed('ct-annotation', true).attr({
                x: 0,
                y: y1,
                ...
                rule = g.append('line').classed('ct-boundary ct-horizontal', true).attr({
                x1: 0,
    ...
    this.groupings.forEach(function (d) {
        if (d.IDs.length) {
            var nIDs = d.IDs.length,
                ...
                y1 = _this.y(firstID),
                y2 = _this.y(d.IDs[0]),
                g = _this.svg.append('g').classed('ct-grouping ct-vertical', true).attr('id', d.key.replace(/ /g, '-')),
                topBoundary = g.append('line').classed('ct-boundary ct-horizontal', true).attr({
                x1: _this.plot_width,
                x2: _this.plot_width + _this.margin.right / 8,
                y1: y1 + 3 * _this.y.rangeBand() / 4,
                y2: y1 + 3 * _this.y.rangeBand() / 4
            }),
                span = g.append('line').classed('ct-boundary ct-vertical', true).attr({
                x1: _this.plot_width + _this.margin.right / 8,
                x2: _this.plot_width + _this.margin.right / 8,
                y1: y1 + 3 * _this.y.rangeBand() / 4,
                y2: y2 + _this.y.rangeBand()
            }),
                bottomBoundary = g.append('line').classed('ct-boundary ct-horizontal', true).attr({
                x1: _this.plot_width,
                x2: _this.plot_width + _this.margin.right / 8,
            ...
            }).text(_this.config.groupings.filter(function (grouping) {
...
function annotateGrouping() {
    ...
    if (this.config.y.grouping) {
        if (this.config.grouping_direction === 'horizontal') horizontally.call(this);else if (this.config.grouping_direction === 'vertical') vertically.call(this);
    }
}
...
function addStriping() {
    ...
    var yAxisGridLines = this.svg.selectAll('.y.axis .tick').filter(function (d) {
        return !/-g\d+-/.test(d);
    }).each(function (d, i) {
        //Offset tick label.
        ...
        d3.select(this).insert('rect', ':first-child').classed('ct-stripe', true).attr({
            id: d,
...
function offsetCircles(mark, markData) {
    ...
    if (this.raw_data.length && this.raw_data[0].hasOwnProperty(this.config.offset_col)) {
        this.svg.selectAll('g.point').each(function (d) {
            d3.select(this).attr('transform', 'translate(0,' + d.offset * context.config.mark_thickness * 2 + ')');
        });
    } else {
        ...
        }).entries(markData).filter(function (d) {
            return d.values.n > 1;
        });
        ...
        overlapping.forEach(function (d) {
            ...
            d.values.keys.forEach(function (di, i) {
                ...
                var g = d3.select(_this.clinicalTimelines.document.getElementsByClassName(className)[0]);
...
function offsetLines(mark, markData) {
    ...
    if (this.config.offset_col && this.raw_data.length && this.raw_data[0].hasOwnProperty(this.config.offset_col)) {
        ...
        var IDdata = d3.nest().key(function (d) {
        ...
        IDdata.forEach(function (IDdatum) {
            ...
            var lineCoordinates = lineData.map(function (di) {
                ...
                overlappingLines = lineData.filter(function (lineDatum) {
                var overlap = lineCoordinates.filter(function (lineCoordinate) {
                    return lineCoordinate[0] <= lineDatum.x1 && lineCoordinate[1] >= lineDatum.x1 || lineDatum.x1 <= lineCoordinate[0] && lineDatum.x2 >= lineCoordinate[0] || lineCoordinate[0] <= lineDatum.x2 && lineCoordinate[1] >= lineDatum.x2 || lineDatum.x1 <= lineCoordinate[1] && lineDatum.x2 >= lineCoordinate[1];
                });
                ...
                overlappingLines.forEach(function (currentLine, i) {
                    if (i === 0) {
                        ...
                        currentlyOverlappingLines.forEach(function (d) {
                            ...
                            d.overlapping = currLapsPrevX1 || currLapsPrevX2 || currLapsPrev || prevLapsCurrX1 || prevLapsCurrX2 || prevLapsCurr;
                        });
                    ...
                    var g = d3.select(_this.clinicalTimelines.document.getElementsByClassName(className)[0]);
                    g.attr('transform', currentLine.offset > 0 ? 'translate(0,' + currentLine.offset * _this.config.mark_thickness * 2 + ')' : 'translate(0,0)');
                });
            }
        });
    }
...
function offsetOverlappingMarks() {
    ...
    this.config.marks.forEach(function (mark, i) {
        ...
        if (_this.config.ongo_col && _this.raw_data.length && _this.raw_data[0].hasOwnProperty(_this.config.ongo_col)) markData.forEach(function (d) {
            d.ongoing = mark.type === 'line' ? d.values[0].values.raw[0][_this.config.ongo_col] : d.values.raw[0][_this.config.ongo_col];
        });

        //Attach offset value to each mark datum.
        if (_this.config.offset_col && _this.raw_data.length && _this.raw_data[0].hasOwnProperty(_this.config.offset_col)) markData.forEach(function (d) {
            d.offset = mark.type === 'line' ? d.values[0].values.raw[0][_this.config.offset_col] : d.values.raw[0][_this.config.offset_col];
        });

        if (mark.type === 'line') offsetLines.call(_this, mark, markData);else if (mark.type === 'circle') offsetCircles.call(_this, mark, markData);
    });
}
...
function highlightMarks() {
    ...
    //Select marks.
    var highlightedMarks = this.svg.selectAll('.wc-data-mark, .ct-ongoing-event').classed('ct-highlighted', function (d) {
    ...
    paths.each(function (d, i) {
        ...
        var x2 = context.x(context.config.time_function(d.values[1].key)) + (d.ongoing === 'Y' ? context.config.marks.find(function (mark) {
            return mark.type === 'line';
        }).attributes['stroke-width'] / 2 : 0);
        var y = context.y(d.values[0].values.raw[0][context.config.y.column === context.config.id_col ? context.config.id_col : context.config.seq_col]) + context.y.rangeBand() / 2;
        var color = context.config.event_highlight_color;
        var line = g.append('line').classed('ct-highlight-overlay', true).attr({
            x1: x1,
            x2: x2,
            y1: y,
            y2: y,
    ...
    var circles = highlightedMarks.filter(function () {
        return this.tagName === 'circle' && this.getAttribute('class').indexOf('ct-highlighted') > -1;
    });
    circles.attr({
...
function drawOngoingMarks() {
    ...
    if (this.raw_data.length && this.raw_data[0].hasOwnProperty(this.config.ongo_col)) {
        ...
        this.svg.selectAll('.line-supergroup .line').filter(function (d) {
            ...
            var length = x + (highlight ? lineSettings.attributes['stroke-width'] * 1.5 : lineSettings.attributes['stroke-width'] * 1.5);
            var heightOffset = highlight ? lineSettings.attributes['stroke-width'] * 2 / 3 : lineSettings.attributes['stroke-width'] * 2 / 3;
            var arrow = [[length, y], [x, y - heightOffset], [x, y + heightOffset]];

            g.insert('polygon', 'line').datum(d).classed('ct-ongoing-event', true).classed('ct-highlighted', highlight).attr({
                points: arrow.map(function (coordinate) {
                    return coordinate.join(',');
                }).join(' '),
                fill: highlight ? context.config.event_highlight_color : context.colorScale(endpoint.values.raw[0][context.config.event_col]),
...
function addStartStopCircles() {
    ...
    this.svg.selectAll('g.line').each(function (d) {
        var g = d3.select(this);
        d.values.filter(function (di, i) {
            return !(i === 1 && di.values.raw[0][context.config.ongo_col] === context.config.ongo_val);
        }).forEach(function (di) {
            g.append('circle').classed('ct-start-stop-circle', true).attr({
...
function drawPolygon(marks, event_symbol) {
    ...
    marks.each(function (d) {
        ...
        //define vertices
        var vertices = void 0;
        if (event_symbol.symbol === 'square') vertices = [[d.symbolCoordinates.x1, d.symbolCoordinates.y1], [d.symbolCoordinates.x1, d.symbolCoordinates.y3], [d.symbolCoordinates.x3, d.symbolCoordinates.y3], [d.symbolCoordinates.x3, d.symbolCoordinates.y1]];else if (event_symbol.symbol === 'diamond') vertices = [[d.symbolCoordinates.x1, d.symbolCoordinates.y2], [d.symbolCoordinates.x2, d.symbolCoordinates.y3], [d.symbolCoordinates.x3, d.symbolCoordinates.y2], [d.symbolCoordinates.x2, d.symbolCoordinates.y1]];else if (event_symbol.symbol === 'triangle') {
            if (event_symbol.direction === 'right') vertices = [[d.symbolCoordinates.x1, d.symbolCoordinates.y1], [d.symbolCoordinates.x3, d.symbolCoordinates.y2], [d.symbolCoordinates.x1, d.symbolCoordinates.y3]];else if (event_symbol.direction === 'down') vertices = [[d.symbolCoordinates.x1, d.symbolCoordinates.y1], [d.symbolCoordinates.x3, d.symbolCoordinates.y1], [d.symbolCoordinates.x2, d.symbolCoordinates.y3]];else if (event_symbol.direction === 'left') vertices = [[d.symbolCoordinates.x3, d.symbolCoordinates.y1], [d.symbolCoordinates.x1, d.symbolCoordinates.y2], [d.symbolCoordinates.x3, d.symbolCoordinates.y3]];else vertices = [[d.symbolCoordinates.x2, d.symbolCoordinates.y1], [d.symbolCoordinates.x3, d.symbolCoordinates.y3], [d.symbolCoordinates.x1, d.symbolCoordinates.y3]];
        }

        //draw polygon
        var polygon = g.append('polygon').classed('ct-custom-mark', true).attr({
...
function addSymbols() {
    ...
    if (this.config.event_symbols && this.config.event_symbols.length) {
        this.config.event_symbols.forEach(function (event_symbol) {
            var marks = _this.svg.selectAll('g.point').filter(function (d) {
                var event$$1 = d.values.raw[0][_this.config.event_col];
                return (event_symbol.event === event$$1 || event_symbol.events.indexOf(event$$1) > -1) && _this.x_dom[0] <= d.total && d.total <= _this.x_dom[1];
            }).each(function (d) {
...
function addSymbolsToLegend() {
    ...
    if (this.config.event_symbols && this.config.event_symbols.length) {
        this.config.event_symbols.forEach(function (event_symbol) {
            var marks = _this.wrap.selectAll('.legend .legend-color-block').filter(function (d) {
                var event$$1 = d.label;
                return event_symbol.event === event$$1 || event_symbol.events.indexOf(event$$1) > -1;
            }).each(function (d) {
...
function offsetBottomXaxis() {
    if (!this.clinicalTimelines.test) {
        ...
        bottomXaxis = this.svg.select('.x.axis'),
            ...
            bottomXaxisTransformCoordinates = bottomXaxisTransform.indexOf(',') > -1 ? bottomXaxisTransform.split(',') : bottomXaxisTransform.split(' '),

        //capture x-axis title and its translation coordinates
        bottomXaxisTitle = bottomXaxis.select('.axis-title'),
            bottomXaxisTitleTransform = bottomXaxisTitle.attr('transform').replace(/^translate\((.*)\)$/, '$1'),
            bottomXaxisTitleTransformCoordinates = bottomXaxisTitleTransform.indexOf(',') > -1 ? bottomXaxisTitleTransform.split(',') : bottomXaxisTitleTransform.split(' ');

        //offset x-axis
        bottomXaxis.attr('transform', 'translate(' + +bottomXaxisTransformCoordinates[0] + ',' + (+bottomXaxisTransformCoordinates[1] + this.y.rangeBand()) + ')');

        //offset x-axis title
        bottomXaxisTitle.attr('transform', 'translate(' + +bottomXaxisTitleTransformCoordinates[0] + ',' + (+bottomXaxisTitleTransformCoordinates[1] - 7 * this.margin.bottom / 16) + ')');
    }
}

function addVisibleLine(reference_line) {
    reference_line.visibleLine = reference_line.g.append('line').datum(reference_line.lineDatum).classed('ct-visible-reference-line', true).attr({
        x1: function x1(d) {
            return d.x1;
        },
        x2: function x2(d) {
...
function addHoverLine(reference_line) {
    reference_line.hoverLine = reference_line.g.append('line').datum(reference_line.lineDatum).classed('ct-hover-line', true).attr({
        x1: function x1(d) {
            return d.x1;
        },
        x2: function x2(d) {
...
function updateText(reference_line) {
    reference_line.textDirection = reference_line.lineDatum.x1 <= this.plot_width / 2 ? 'right' : 'left';
    reference_line.text.attr({
...
function addHover(reference_line) {
    ...
    reference_line.hoverLine.on('mouseover', function () {
...
function updateTable(reference_line) {
    ...
    reference_line.wide_data = this.filtered_wide_data.filter(function (d) {
        return _this.config.time_function(d[_this.config.st_col]) <= _this.config.time_function(reference_line.timepoint) && _this.config.time_function(d[_this.config.en_col]) >= _this.config.time_function(reference_line.timepoint);
    });

    //Nest data by grouping and event type.
    reference_line.nested_data = d3.nest().key(function (d) {
    ...
    reference_line.nested_data.forEach(function (d) {
        reference_line.flattened_data.push({
            group: d.key,
            group_n: d3.sum(d.values, function (di) {
                return di.values;
            }),
    ...
    reference_line.flattened_data.sort(function (a, b) {
        var groupOrder = _this.config.reference_table_sort === 'frequency' ? b.group_n - a.group_n : a.group < b.group ? -1 : a.group > b.group ? 1 : 0;
        var classOrder = a.class === 'ct-higher-level' ? -1 : b.class === 'ct-higher-level' ? 1 : 0;
        var lowerOrder = _this.config.reference_table_sort === 'frequency' ? b.n - a.n : _this.config.reference_table_sort === 'event' ? _this.config.color_dom.indexOf(a.key) - _this.config.color_dom.indexOf(b.key) : a.key < b.key ? -1 : 1;
        return groupOrder || classOrder || lowerOrder;
    });

    //Update table.
    reference_line.table.selectAll('tr').remove();
    reference_line.table.selectAll('tr').data(reference_line.flattened_data).enter().append('tr').each(function (d) {
        var row = d3.select(this);
        row.append('td').text(d.key).attr('class', function (d) {
            return d.class + (d.class === 'ct-lower-level' ? ' ct-indent' : '');
        }).style('font-weight', 'bold').style('color', function (d) {
            return d.class === 'ct-lower-level' ? context.colorScale(d.key) : 'black';
        });
        row.append('td').text(d.n).attr('class', function (d) {
            return d.class;
        }).style('font-weight', 'bold').style('color', function (d) {
            return d.class === 'ct-lower-level' ? context.colorScale(d.key) : 'black';
        });
...
function drawReferenceLine(reference_line, i) {
    reference_line.g = this.referenceLinesGroup.append('g').classed('ct-reference-line', true).attr('id', 'ct-reference-line-' + i);
    ...
    reference_line.lineDatum = {
        ...
        y1: 0,
        y2: this.plot_height + (this.config.y.column === this.config.id_col ? this.y.rangeBand() : 0)
    };
...
function drawReferenceTable(reference_line, i) {
    ...
    reference_line.tableContainer = this.clinicalTimelines.containers.leftColumn.append('div').classed('ct-reference-line-table-container', true).attr('id', 'ct-reference-line-table-container-' + i);

    //Add reference line table header.
    reference_line.tableHeader = reference_line.tableContainer.append('h3').classed('ct-reference-line-header', true);

    //Add reference line table.
    reference_line.table = reference_line.tableContainer.append('div').classed('ct-reference-line-table-body', true).append('table').append('tbody');
...
function drawReferenceLines() {
    ...
    if (this.config.reference_lines) {
        ...
        this.clinicalTimelines.containers.leftColumn.selectAll('.ct-reference-line-label-container').remove();

        //Add group for reference lines.
        this.svg.select('.ct-reference-lines').remove();
        if (!this.parent) this.clinicalTimelines.containers.leftColumn.selectAll('.ct-reference-line-table-container').remove();
        this.referenceLinesGroup = this.svg.insert('g', '#clinical-timelines .wc-chart .wc-svg .line-supergroup').classed('ct-reference-lines', true);

        //Append reference line for each item in config.reference_lines.
        this.config.reference_lines.filter(function (reference_line) {
            return reference_line.time_scale === _this.config.time_scale;
        }).filter(function (reference_line) {
            return _this.x_dom[0] <= _this.config.time_function(reference_line.timepoint) && _this.x_dom[1] >= _this.config.time_function(reference_line.timepoint);
...
function IEsucks() {
    ...
    if (inIE) this.svg.selectAll('.line,.point').each(function (d) {
...
function timelines() {
    //Create timelines.
    this.timelines = webcharts.createChart(this.containers.timelines.node(), this.settings.synced, this.controls);
...
function drawTimeRange() {
    ...
    var x_dom = this.x_dom.map(function (x) {
        ...
        timeRange = this.parent.timelines.config.time_scale === 'day' ? this.parent.timelines.day_range : this.parent.timelines.date_range.map(function (dt) {
        return dt.getTime();
    }),
        timeRangeText = this.config.time_scale === 'day' ? this.parent.timelines.day_range.map(function (dy) {
        return dy.toString();
    ...
    if ((x_dom[0] !== timeRange[0] || x_dom[1] !== timeRange[1]) && (timeRange[0] >= x_dom[0] && timeRange[0] <= x_dom[1] || // left side of time range overlaps x-domain
    ...
    timeRange[0] <= x_dom[0] && timeRange[1] >= x_dom[1]) // time range fully encompassed x-domain
            ...
            var timeRangeGroup = this.svg.insert('g', '#clinical-timelines .wc-chart .wc-svg .line-supergroup').classed('ct-time-range', true),
                x = this.x(Math.max(timeRange[0], x_dom[0])),
                width = Math.min(this.x(timeRange[1]) - x, this.plot_width - x),
                timeRangeRect = timeRangeGroup.append('rect').classed('ct-time-range-rect', true).attr({
                x: x,
                y: 0,
                width: width,
                height: this.plot_height
            }),
                timeRangeTooltip = timeRangeGroup.append('title').text(this.config.time_scalePropCased + ' range: ' + timeRangeText.join(' - '));
        }
}
...
function IDtimeline() {
    this.IDtimeline = webcharts.createChart(this.containers.IDtimeline.node(), this.settings.IDtimeline);
