...
function syncSettings(settings) {
    ...
    //Default filters.
    var defaultFilters = [{ value_col: syncedSettings.id_col, label: syncedSettings.id_unitPropCased }, { value_col: syncedSettings.event_col, label: 'Event Type' }, { value_col: syncedSettings.site_col, label: 'Site' }];
    if (syncedSettings.ongo_col) defaultFilters.push({ value_col: syncedSettings.ongo_col, label: 'Ongoing?' });
    ...
    //Default details
    var defaultDetails = [{ value_col: syncedSettings.event_col, label: 'Event Type' }, { value_col: syncedSettings.stdy_col, label: 'Start Day' }, { value_col: syncedSettings.endy_col, label: 'Stop Day' }, { value_col: syncedSettings.seq_col, label: 'Sequence Number' }];
...
function syncControls(controls, settings) {
    settings.filters.reverse().forEach(function (filter) {
        ...
        if ([settings.id_unitPropCased, 'Site'].indexOf(filter.label) > -1) controls.unshift(filter);else controls.splice(controls.length - 3, 0, filter);
    });
...
function onLayout() {
    ...
    //Add top x-axis.
    this.svg.append('g').classed('x-top axis linear', true).append('text').classed('axis-title top', true).text('Study Day');
    ...
    var controls = this.controls.wrap.selectAll('.control-group'),
        filters = controls.filter(function (d) {
        return d.type === 'subsetter';
    }),
        otherControls = controls.filter(function (d) {
        return d.type !== 'subsetter';
    });
    ...
    .each(function (d) {
        if (d.value_col === context.config.event_col) d3$1.select(this).selectAll('option').property('selected', function (di) {
    ...
    otherControls.each(function (d) {
        ...
        if (d.label === 'Y-axis') {
            ...
            else if (d.description === 'grouping') options.property('label', function (di) {
...
function groupingData() {
    ...
    //Calculate x-domain.
    var xDomain = [d3.min(this.raw_data, function (d) {
    ...
    this.groupings = d3$1.set(this.raw_data.filter(function (d) {
        var filtered = false;
    ...
    //range_band hack
    if (this.config.grouping_direction === 'horizontal') this.config.range_band = this.initialSettings.range_band + this.groupings.length * 2 / d3$1.set(this.wide_data.map(function (d) {
        return d[_this.config.id_col];
...
function onDatatransform() {
    ...
    this.populationDetails.wrap.html('<span class = \'stats\'>' + this.populationDetails.n + '</span> of <span class = \'stats\'>' + this.populationDetails.N + '</span> ' + this.config.id_unit + '(s) displayed (<span class = \'stats\'>' + d3$1.format('%')(this.populationDetails.rate) + '</span>)');
}
...
function sortYdomain() {
    ...
    if (this.config.y.sort === 'earliest') {
        ...
        if (this.config.y.grouping) {
            this.y_dom = d3$1.set(filtered_data.map(function (d) {
                return d[_this.config.id_col];
...
function onDraw() {
    ...
    if (this.config.y.grouping && this.config.grouping_direction === 'vertical') this.config.margin.right = 40;else delete this.config.margin.right;
}
...
function drawReferenceLines() {
    ...
    this.config.referenceLines.forEach(function (studyDay, i) {
        ...
        direction = studyDay.studyDay <= (_this.x_dom[1] - _this.x_dom[0]) / 2 ? 'right' : 'left',
...
function onResize() {
    ...
    this.wrap.selectAll('.legend-item').filter(function (d) {
        return d.label === 'None';
    }).classed('hidden', true);
...
function listing(clinicalTimelines) {
    var listing = webcharts.createTable(clinicalTimelines.element, clinicalTimelines.config.details_config);
