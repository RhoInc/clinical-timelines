...
function syncRendererSpecificSettings(settings) {
    ...
    settings.date_ranges = settings.date_ranges.filter(function (date_range) {
    ...
    settings.day_ranges = settings.day_ranges.filter(function (day_range) {
...
function defineData() {
    ...
    var timepoints = this.wide_data.filter(function (d) {
        ...
        timeIntervals = lengthenRaw(this.wide_data.filter(function (d) {
        return d[_this.config.st_col] !== d[_this.config.en_col];
...
function updateTimeRangeControls() {
    var timeRanges = [this.full_date_time_range, this.date_time_range, this.full_day_time_range, this.day_time_range];
    this.controls.config.inputs.find(function (input) {
        return input.option === 'time_range';
    }).values = ['custom'].concat(timeRanges);
}
...
function checkOtherControls() {
    ...
    this.controls.config.inputs.filter(function (input) {
        ...
        if (input.description === 'Event highlighting') input.values = _this.config.color_dom;else if (input.description === 'Y-axis grouping') input.values = _this.config.groupings.map(function (grouping) {
            return grouping.value_col;
        });
...
function checkFilters() {
    ...
    this.controls.config.inputs = this.controls.config.inputs.filter(function (input) {
        if (input.type !== 'subsetter') return true;else if (!_this.raw_data[0].hasOwnProperty(input.value_col)) {
...
function timeScale(dropdown, d) {
    ...
    if (this.controls.config.inputs.find(function (input) {
        return input.option === 'time_range';
    }).values.indexOf(this.config.time_range) < 0) this.config.time_range = 'custom';
...
function timeRange(dropdown, d) {
    ...
    this.time_range = this.config.time_range.split(' - ').map(function (d) {
        return _this.config.time_function(d);
    });
...
function onChange(input, d) {
    ...
    this.config.time_range = this.time_range.map(function (d) {
        return d instanceof Date ? d3.time.format(_this.config.date_format)(d) : '' + d;
    }).join(' - ');
    this[this.config.time_scale + '_time_range'] = this.config.time_range;
...
function drawTimeRange() {
    ...
    timeRange[0] <= x_dom[0] && timeRange[1] >= x_dom[1]) // time range fully encompassed x-domain
            ...
            var timeRangeGroup = this.svg.insert('g', '#clinical-timelines .wc-chart .wc-svg .line-supergroup').classed('ct-time-range', true),
                ...
                timeRangeTooltip = timeRangeGroup.append('title').text(this.parent.timelines.config.x.label + ' Range: ' + timeRangeText.join(' - '));
        }
}
