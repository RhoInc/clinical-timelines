...
function updatePopulationDetails() {
    this.clinicalTimelines.containers.popCurrentFilters.classed('ct-hidden', this.populationDetails.nOutsideTimeRange).html('<span class = \'ct-stats ct-sample\'>' + this.populationDetails.n + '</span> of <span class = \'ct-stats\'>' + this.populationDetails.N + '</span> ' + (this.populationDetails.N > 1 ? this.config.id_unitPlural : this.config.id_unit) + ' (<span class = \'ct-stats\'>' + d3$1.format('%')(this.populationDetails.rate) + '</span>) <span class = \'ct-info-icon\' title = \'These ' + this.config.id_unitPlural + ' have data that meet the current filter criteria.\'>&#9432;</span>');
...
function addStriping() {
    ...
    var yAxisGridLines = this.svg.selectAll('.y.axis .tick').each(function (d, i) {
        //Offset tick label.
        d3$1.select(this).select('text').attr('dy', context.y.rangeBand() / 3);
