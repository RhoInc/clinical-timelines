...
function onInit() {
    ...
    this.controls.config.inputs = this.controls.config.inputs.filter(function (filter) {
        if (filter.type !== 'subsetter') return true;else {
...
function backButton() {
    ...
    }).selectAll('.radio input').property('disabled', false);
...
function drawParticipantTimeline() {
        ...
        this.controls.wrap.selectAll('.control-group').filter(function (control) {
                return control.label === 'Sort participants';
        }).selectAll('.radio input').property('disabled', !!this.selected_id);
}
...
function drawParticipantTimeline$1() {
    ...
    this.controls.wrap.selectAll('.control-group').filter(function (control) {
        return control.label === 'Sort participants';
    }).selectAll('.radio input').property('disabled', true);
        ...
        if (overlappingLines.length) {
            ...
            overlappingLines.forEach(function (currentLine, i) {
                ...
                if (i === 0) {
                    ...
                    var nOverlapping = currentlyOverlappingLines.length;

                    for (var j = 0; j < nOverlapping; j++) {
                        previousLine = currentlyOverlappingLines[j];

                        var currLapsPrevX1 = currentLine.x1 <= previousLine.x1 && currentLine.x2 >= previousLine.x1,
                        ...
                        if (currLapsPrevX1 || currLapsPrevX2 || currLapsPrev || prevLapsCurrX1 || prevLapsCurrX2 || prevLapsCurr) {
                            console.log('if');
                            currentLine.offset = d3.max(currentlyOverlappingLines, function (lineDatum) {
                                return lineDatum.offset;
                            }) + 1;
                            currentlyOverlappingLines.push(currentLine);
                            console.log(currentlyOverlappingLines.map(function (d) {
                                return d.offset;
                            }));
                            break;
                        } else {
