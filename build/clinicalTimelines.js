...
function onPreprocess() {
    ...
    if (this.config.y.grouping) defineGroupingData.call(this);else delete this.groupings;
...
function highlightMarks() {
    ...
    var highlightedMarks = this.svg.selectAll('.wc-data-mark, .ongoing-event').classed('highlighted', function (d) {
        ...
        paths = highlightedMarks.filter(function () {
        ...
        circles = highlightedMarks.filter(function () {
    ...
    if (this.config.reference_lines) {
        ...
        this.config.reference_lines.filter(function (reference_line) {
            ...
            var referenceLineGroup = referenceLinesGroup.append('g').classed('reference-line', true).attr('id', 'reference-line-' + i),
                ...
                y1: 0,
                y2: _this.plot_height + _this.y.rangeBand()
            }),
...
function drawTimeRange() {
    ...
    var x_dom = this.x_dom.map(function (x) {
        ...
        timeRange = this.parent.clinicalTimelines.config.time_scale === 'Study Day' ? this.parent.clinicalTimelines.config.study_day_range : this.parent.clinicalTimelines.config.date_range.map(function (dt) {
        return dt.getTime();
    }),
        timeRangeText = this.config.time_scale === 'Study Day' ? this.parent.clinicalTimelines.config.study_day_range.map(function (dy) {
        return dy.toString();
