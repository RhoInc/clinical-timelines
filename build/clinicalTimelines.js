...
function defineStyles() {
    ...
    //Grouping
    '#clinical-timelines > #right-side > .wc-chart .wc-svg .grouping .boundary {' + '    stroke: black;' + '    stroke-width: 2px;' + '}', '#clinical-timelines > #right-side > .wc-chart .wc-svg .grouping .annotation {' + '    font-size: 24px;' + '    font-weight: bold;' + '    text-anchor: beginning;' + '}', '#clinical-timelines > #right-side > .wc-chart .wc-svg .grouping.vertical .annotation {' + '    writing-mode: tb-rl;' + '}',

    //Y-axis
    '#clinical-timelines > #right-side > .wc-chart .wc-svg .y.axis .tick {' + '    cursor: pointer;' + '    fill: blue;' + '    text-decoration: underline;' + '}',
...
function onInit() {
    ...
    this.config.date_range = this.config.date_range instanceof Array && this.config.date_range.length === 2 ? this.config.date_range.map(function (date) {
        return d3.time.format(_this.config.date_format).parse(date);
...
function augmentFilterControls() {
    ...
    .each(function (d) {
        ...
        if (filter.col === _this.config.id_col) {
            ...
            if (_this.selected_id) drawIDtimeline.call(_this);
        }
    });
}

function eventHighlightingChange(dropdown, d) {
    //Update event highlighting settings.
    this.config.event_highlighted = d3.select(dropdown).select('option:checked').text();
...
function groupingData() {
    ...
    //Calculate x-domain.
    var xDomain = this.config.time_scale === 'Study Day' ? [d3.min(this.raw_data, function (d) {
        return +d[_this.config.stdy_col];
    }), d3.max(this.raw_data, function (d) {
        return +d[_this.config.endy_col];
    })] : [d3.min(this.raw_data, function (d) {
    ...
    this.groupings = d3.set(this.raw_data.filter(function (d) {
        var filtered = false;

        _this.filters.forEach(function (di) {
            if (filtered === false && di.val !== 'All' && d[_this.config.event_col] !== 'Grouping') {
    ...
    //range_band hack
    if (this.config.grouping_direction === 'horizontal') this.config.range_band = this.initialSettings.range_band + this.groupings.length * 2 / d3.set(this.wide_data.filter(function (d) {
        var filtered = false;
        ...
        return d[_this.config.id_col];
    })).values().length * this.initialSettings.range_band;
}
...
function onPreprocess() {
    ...
    this.true_filtered_data = this.raw_data.filter(function (d) {
        var filtered = false;
    ...
    if (this.config.y.grouping) groupingData.call(this);else {
...
function onDatatransform() {
    ...
    this.populationDetails.wrap.html('<span class = \'stats\'>' + this.populationDetails.n + '</span> of <span class = \'stats\'>' + this.populationDetails.N + '</span> ' + this.config.id_unit + '(s) (<span class = \'stats\'>' + d3.format('%')(this.populationDetails.rate) + '</span>)');
}
...
function sortYdomain() {
    ...
    if (this.config.y.sort === 'earliest') {
        ...
        if (this.config.y.grouping) {
            ...
            }).entries(filtered_data).sort(function (a, b) {
        ...
        if (this.config.y.grouping) {
            this.y_dom = d3.set(filtered_data.map(function (d) {
                return d[_this.config.id_col];
...
function drawTopXaxis() {
    var topXaxis = d3.svg.axis().scale(this.x).orient('top').tickFormat(this.config.time_scale === 'Date' ? d3.time.format(this.config.date_format) : d3.format('1d')).innerTickSize(this.xAxis.innerTickSize()).outerTickSize(this.xAxis.outerTickSize()).ticks(this.xAxis.ticks()[0]),
...
function onInit$1() {
    ...
    this.config.x.domain = [d3.min(this.parent.clinicalTimelines.raw_data, function (d) {
        return _this.config.time_scale === 'Study Day' ? +d[_this.config.stdy_col] : d3.time.format(_this.config.date_format).parse(d[_this.config.stdt_col]);
    }), d3.max(this.parent.clinicalTimelines.raw_data, function (d) {
        return _this.config.time_scale === 'Study Day' ? +d[_this.config.endy_col] : d3.time.format(_this.config.date_format).parse(d[_this.config.endt_col]);
    })];
