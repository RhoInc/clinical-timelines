...
function defineStyles() {
    ...
    '#clinical-timelines .hidden {' + '    display: none !important;' + '}', '#clinical-timelines .ct-button {' + '    cursor: pointer !important;' + '    border-radius: 4px !important;' + '    padding: 5px !important;' + '}', '#clinical-timelines .ct-button.highlighted {' + '    border: 2px solid black !important;' + '}', '#clinical-timelines .ct-button.selected {' + '    background: lightgray;' + '}',

    /***--------------------------------------------------------------------------------------\
      Left and right side containers
    \--------------------------------------------------------------------------------------***/

    '#clinical-timelines > * {' + '    display: inline-block;' + '    padding: .5%;' + '}', '#clinical-timelines > #left-side {' + '    width: 22%;' + '    float: left;' + '}', '#clinical-timelines > #right-side {' + '    width: 75%;' + '    float: right;' + '}', '#clinical-timelines > * > * {' + '    width: 100%;' + '    padding: 1%;' + '    vertical-align: top;' + '    border: 1px solid #eee;' + '    display: inline-block;' + '    margin-bottom: 10px;' + '}',
    ...
    //Lines
    '#clinical-timelines path.wc-data-mark {' + '    stroke-width: 4;' + '    clip-path: url(#1);' + '    stroke-opacity: 1;' + '}', '#clinical-timelines path.wc-data-mark.highlighted {' + '    stroke-width: 7;' + '}', '#clinical-timelines line.highlight-overlay {' + '    stroke-width: 3;' + '    stroke-linecap: round;' + '}',
...
function syncRendererSpecificSettings(settings) {
    ...
    //Filters
    var defaultFilters = [{ value_col: settings.id_col, label: settings.id_unitPropCased }, { value_col: settings.site_col, label: 'Site' }, { value_col: settings.event_col, label: 'Event Type' }];
    ...
    if (settings.reference_lines) {
        ...
        settings.reference_lines = settings.reference_lines.map(function (referenceLine) {
            var referenceLineObject = {};
            referenceLineObject.studyDay = referenceLine.studyDay || referenceLine;
            referenceLineObject.label = referenceLine.label || 'Study Day ' + referenceLineObject.studyDay;
    ...
    //Details
    var defaultDetails = settings.time_scale === 'Study Day' ? [{ value_col: settings.event_col, label: 'Event Type' }, { value_col: settings.stdy_col, label: 'Start Day' }, { value_col: settings.endy_col, label: 'Stop Day' }, { value_col: settings.seq_col, label: 'Sequence Number' }] : [{ value_col: settings.event_col, label: 'Event Type' }, { value_col: settings.stdt_col, label: 'Start Date' }, { value_col: settings.endt_col, label: 'Stop Date' }, { value_col: settings.seq_col, label: 'Sequence Number' }];
...
function syncTimeScaleSettings(settings) {
    ...
    settings.marks[0].tooltip = settings.time_scale === 'Study Day' ? 'Event: [' + settings.event_col + ']' + ('\nStart Day: [' + settings.stdy_col + ']') + ('\nStop Day: [' + settings.endy_col + ']') : 'Event: [' + settings.event_col + ']' + ('\nStart Date: [' + settings.stdt_col + ']') + ('\nStop Date: [' + settings.endt_col + ']');
    settings.marks[0].values = settings.time_scale === 'Study Day' ? { wc_category: [settings.stdy_col, settings.endy_col] } : { wc_category: [settings.stdt_col, settings.endt_col] };

    //Circles (events without duration)
    settings.marks[1].tooltip = settings.time_scale === 'Study Day' ? 'Event: [' + settings.event_col + ']' + ('\nStudy Day: [' + settings.stdy_col + ']') : 'Event: [' + settings.event_col + ']' + ('\nStudy Date: [' + settings.stdt_col + ']');
    settings.marks[1].values = settings.time_scale === 'Study Day' ? { wc_category: ['DY'] } : { wc_category: ['DT'] };
...
function defineData() {
    ...
    var singleDayEvents = this.wide_data.filter(function (d) {
        return d[_this.config.stdy_col] === d[_this.config.endy_col] || d[_this.config.stdt_col] === d[_this.config.endt_col];
    }).map(function (d) {
        d.wc_category = _this.config.time_scale === 'Study Day' ? 'DY' : 'DT';
        d.wc_value = _this.config.time_scale === 'Study Day' ? d[_this.config.stdy_col] : d[_this.config.stdt_col];
        return d;
    }),
        multiDayEvents = lengthenRaw(this.wide_data.filter(function (d) {
        return d[_this.config.stdy_col] !== d[_this.config.endy_col] || d[_this.config.stdt_col] !== d[_this.config.endt_col];
    }), this.config.time_scale === 'Study Day' ? [this.config.stdy_col, this.config.endy_col] : [this.config.stdt_col, this.config.endt_col]);
...
function handleEventTypes() {
    ...
    this.config.color_dom = this.currentEventTypes !== 'All' ? this.currentEventTypes.concat(this.allEventTypes.filter(function (eventType) {
        return _this.currentEventTypes.indexOf(eventType) === -1;
    }).sort()) : this.allEventTypes;
...
function removeSiteReferences() {
    ...
    if (!this.raw_data[0].hasOwnProperty(this.config.site_col)) {
        ...
        var yAxisGrouping = this.controls.config.inputs.filter(function (input) {
        ...
        this.config.id_characteristics = this.config.id_characteristics.filter(function (id_characteristic) {
...
function onInit() {
    ...
    this.raw_data.forEach(function (d) {
        ...
        d[_this.config.stdt_col] = /^\d{4}-\d\d-\d\d *$/.test(d[_this.config.stdt_col]) ? d[_this.config.stdt_col] : '';
        d[_this.config.endt_col] = /^\d{4}-\d\d-\d\d *$/.test(d[_this.config.endt_col]) ? d[_this.config.endt_col] : d[_this.config.stdt_col];
    });
    ...
    this.wide_data = this.raw_data.filter(function (d) {
        return d[_this.config.stdy_col] !== NaN && d[_this.config.endy_col] !== NaN && !(d.hasOwnProperty(_this.config.stdt_col) && d[_this.config.stdt_col] == '') && !(d.hasOwnProperty(_this.config.endt_col) && d[_this.config.endt_col] == '') && !/^\s*$/.test(d[_this.config.id_col]) && // remove records with missing [id_col]
        !/^\s*$/.test(d[_this.config.event_col]);
    } // remove records with missing [event_col]
    );

    //Define x-domain.
    this.config.study_day_range = this.config.study_day_range || [d3.min(this.raw_data, function (d) {
        return d[_this.config.stdy_col];
    }), d3.max(this.raw_data, function (d) {
        return d[_this.config.endy_col];
    })];
    this.config.date_range = this.config.date_range instanceof Array && this.config.date_range.length === 2 ? this.config.date_range.map(function (date) {
...
function augmentFilters() {
    ...
    .each(function (d) {
        if (d.value_col === context.config.event_col) d3.select(this).selectAll('option').property('selected', function (di) {
            return context.currentEventTypes instanceof Array ? context.currentEventTypes.indexOf(di) > -1 : true;
        });
    }).on('change', function (d) {
        if (d.value_col === context.config.id_col) IDchange.call(context, this, d);else if (d.value_col === context.config.event_col) eventTypeChange.call(context, this, d);
    });
}
...
function onLayout() {
    ...
    this.IDdetails.wrap.append('div').html(this.config.id_unitPropCased + ': <span id = \'ID\'></span>');
...
function definePopulationDetails() {
    ...
    this.populationDetails.sample = d3.set(this.filtered_wide_data.map(function (d) {
        ...
        IDobject.dataInsideTimeRange = IDobject.data.filter(function (d) {
            var st = _this.config.time_scale === 'Study Day' ? +d[_this.config.stdy_col] : d3.time.format(_this.config.date_format).parse(d[_this.config.stdt_col]),
                en = _this.config.time_scale === 'Study Day' ? +d[_this.config.endy_col] : d3.time.format(_this.config.date_format).parse(d[_this.config.endt_col]),
...
function onPreprocess() {
    this.config.x.domain = this.config.time_scale === 'Study Day' ? this.config.study_day_range : this.config.date_range;
...
function sortYdomain() {
    ...
    if (this.config.y.sort === 'earliest') {
        if (this.config.y.grouping) {
            ...
            var nestedData = d3.nest().key(function (d) {
                ...
                return d3.min(d, function (di) {
                    return _this.config.time_scale === 'Study Day' ? +di[_this.config.stdy_col] : d3.time.format(_this.config.date_format).parse(di[_this.config.stdt_col]);
                });
        ...
        if (this.config.y.grouping) {
            ...
            this.y_dom = d3.set(this.longDataInsideTimeRange.map(function (d) {
                return d[_this.config.id_col];
...
function onDraw() {
    ...
    if (this.config.grouping_direction === 'vertical') {
        if (this.config.y.grouping) this.config.margin.right = 40;else delete this.config.margin.right;
    }
...
function drawTopXaxis() {
    var topXaxis = d3.svg.axis().scale(this.x).orient('top').tickFormat(this.config.time_scale === 'Date' ? d3.time.format(this.config.date_display_format) : d3.format('1d')).innerTickSize(this.xAxis.innerTickSize()).outerTickSize(this.xAxis.outerTickSize()).ticks(this.xAxis.ticks()[0]),
        topXaxisSelection = this.svg.select('g.x-top.axis').attr('class', 'x-top axis linear');
    topXaxisSelection.call(topXaxis);
    topXaxisSelection.select('text.axis-title.top').attr('transform', 'translate(' + (this.raw_width / 2 - this.margin.left) + ',-' + 9 * this.config.margin.top / 16 + ')');
}
    ...
    if (this.config.ongo_col && this.raw_data[0].hasOwnProperty(this.config.ongo_col)) {
        ...
        this.svg.selectAll('.line-supergroup .line').filter(function (d) {
            ...
            var g = d3.select(this),
                endpoint = d.values[1],
                x = context.x(context.config.time_scale === 'Study Day' ? +endpoint.key : new Date(endpoint.key)),
...
function highlightMarks() {
    ...
    var highlightedMarks = this.svg.selectAll('.wc-data-mark, .ongoing-event').classed('highlighted', function (d) {
        ...
        var g = d3.select(this.parentNode),
            x1 = context.x(context.config.time_scale === 'Study Day' ? +d.values[0].key : new Date(d.values[0].key)),
            x2 = context.x(context.config.time_scale === 'Study Day' ? +d.values[1].key : new Date(d.values[1].key)),
...
function drawReferenceLines() {
    ...
    if (this.config.reference_lines) {
        ...
        this.config.reference_lines.filter(function (reference_line) {
            ...
            direction = referenceLine.timepoint <= (_this.x_dom[1] - _this.x_dom[0]) / 2 ? 'right' : 'left',
...
function offsetBottomXaxis() {
    ...
    bottomXaxis.attr('transform', 'translate(0,' + (+bottomXaxis.attr('transform').split(',')[1].split(')')[0] + this.y.rangeBand()) + ')');
    bottomXaxisTitle.attr('transform', 'translate(\n            ' + +bottomXaxisTitle.attr('transform').split(',')[0].split('(')[1] + ',\n            ' + (+bottomXaxisTitle.attr('transform').split(',')[1].split(')')[0] - 7 * this.margin.bottom / 16) + ')');
}
...
function onResize$1() {
    ...
    //Highlight marks.
    this.svg.selectAll('.highlight-overlay').remove();
    this.svg.selectAll('.wc-data-mark, .ongoing-event').classed('highlighted', function (d) {
        ...
        var g = d3.select(this.parentNode),
            ...
            x1: context.x(context.config.time_scale === 'Study Day' ? +d.values[0].key : new Date(d.values[0].key)),
            x2: context.x(context.config.time_scale === 'Study Day' ? +d.values[1].key : new Date(d.values[1].key)),
            y1: context.y(d.values[0].values.raw[0][context.config.seq_col]) + context.y.rangeBand() / 2,
            y2: context.y(d.values[0].values.raw[0][context.config.seq_col]) + context.y.rangeBand() / 2,
            stroke: context.colorScale(d.values[0].values.raw[0][context.config.event_col])
        });
    });
}
