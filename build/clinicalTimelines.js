...
function defineStyles() {
    ...
    //Circles
    '#clinical-timelines circle.wc-data-mark {' + '    stroke-width: 0;' + '    fill-opacity: 1;' + '}', '#clinical-timelines circle.wc-data-mark.ct-highlighted {' + '    stroke-opacity: 1;' + ('    stroke-width: ' + circle.attributes['stroke-width'] + ';') + '}',
...
function drawIDtimeline() {
    ...
    }), this.config.event_col, null, clinicalTimelines.test);
...
function offsetOverlappingMarks() {
    ...
    this.config.marks.forEach(function (mark, i) {
        ...
        if (_this.config.ongo_col && _this.raw_data[0].hasOwnProperty(_this.config.ongo_col)) markData.forEach(function (d) {
        ...
        if (_this.config.offset_col && _this.raw_data[0].hasOwnProperty(_this.config.offset_col)) markData.forEach(function (d) {
...
function triangle(marks, direction) {
    var context = this;
    ...
    marks.each(function (d) {
        ...
        //clear out g
        g.select('rect.ct-custom-triangle').remove();
        g.select('circle').classed('ct-hidden', true);

        //draw triangle
        var x = context.x(d.values.x);
        var y = context.y(d.values.y) + context.y.rangeBand() / 2;
        var factor = 3 / 4;
        var triangle = g.append('polygon').classed('ct-custom-triangle', true).attr({
            points: [[x, y - context.config.mark_thickness * factor].join(','), [x, y + context.config.mark_thickness * factor].join(','), [x + context.config.mark_thickness * factor * 2, y].join(',')].join(' '),
...
function square(marks) {
    ...
    marks.each(function (d) {
        ...
        //clear out g
        g.select('rect.ct-custom-square').remove();
        g.select('circle').classed('ct-hidden', true);

        //draw square
        var factor = 3 / 4;
        var x = context.x(d.values.x) - context.config.mark_thickness * factor;
        var y = context.y(d.values.y) + context.y.rangeBand() / 2 - context.config.mark_thickness * factor;
        var width = context.config.mark_thickness * factor * 2;
        var height = context.config.mark_thickness * factor * 2;
        var fill = context.colorScale(d.values.raw[0][context.config.event_col]);
        var square = g.append('rect').classed('ct-custom-square', true).attr({
            x: x,
            y: y,
            width: width,
            height: height,
...
function diamond(marks, direction) {
    var context = this;
    ...
    marks.each(function (d) {
        ...
        var y = context.y(d.values.y) + context.y.rangeBand() / 2;
        var factor = 3 / 4;
        var diamond = g.append('polygon').classed('ct-custom-diamond', true).attr({
            points: [[x, y - context.config.mark_thickness * factor].join(','), [x + context.config.mark_thickness * factor, y].join(','), [x, y + context.config.mark_thickness * factor].join(','), [x - context.config.mark_thickness * factor, y].join(',')].join(' '),
            fill: context.colorScale(d.values.raw[0][context.config.event_col]),
            'clip-path': 'url(#' + context.id + ')',
            stroke: 'black'
        });
...
function addSymbols() {
    ...
    this.config.event_symbols.forEach(function (event_symbol) {
        ...
        if (event_symbol.symbol === 'triangle') triangle.call(_this, marks, event_symbol.direction);else if (event_symbol.symbol === 'square') square.call(_this, marks);else if (event_symbol.symbol === 'diamond') diamond.call(_this, marks);
    });
}
